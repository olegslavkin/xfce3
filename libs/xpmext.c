...
Pixmap duppix (Pixmap pixmap, int width, int height)
{
  Display *tmpDpy;
  Pixmap copyP = 0;
  ...
  if (!(tmpDpy = XOpenDisplay ("")))
  ...
  else
    {
      ...
      copyP =
	XCreatePixmap (tmpDpy, GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()), width,
		       height, DefaultDepth (tmpDpy, DefaultScreen (tmpDpy)));
      XCopyArea (tmpDpy, pixmap, copyP,
		 DefaultGC (tmpDpy, DefaultScreen (tmpDpy)), 0, 0, width,
		 height, 0, 0);
...
setPixmapProperty (Pixmap pixmap)
{
  ...
  XGetWindowProperty (GDK_DISPLAY (), GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()),
		      prop, 0L, 1L, False, AnyPropertyType,
  ...
  if (pixmap)
    {
      XChangeProperty (GDK_DISPLAY (),
		       GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()), prop,
		       XA_PIXMAP, 32, mode, (unsigned char *) &pixmap, 1);
    }
  else
    XDeleteProperty (GDK_DISPLAY (), GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()),
		     prop);
...
void
GradientXpmImage (int sizex, int sizey, XpmImage * Imagedest,
		  int r1, int g1, int b1, int r2, int g2, int b2, int ncolors)
{
  ...
  for (i = 0; i < sizey; i++)
	...
	Imagedest->data[i * sizex + j] =
	  ((v < (ncolors - 1)) ? v : (ncolors - 1));
      }
}
...
void
XpmShapeToFit_dither (int newsizex, int newsizey, XpmImage * I,
		      XpmImage * Imagedest)
{
  int getr, getg, getb;
  int *shrinknumbers;
	  ...
	  for (b = 0; b < 4; b++)
	    {
	      ...
	      sprintf ((Imagedest->colorTable)[i].c_color, "#%X%X%X", getr,
		       getg, getb);
	    }
	}
    }
  ...
  for (y = 0; y < (I->height); y++)
      ...
      for (x = 0; x < (I->width); x++)
	{
	  if (myshape[I->data[k]] == False)
	    {
	      i =
		(int) ((newsizex - 1) * (double) x / (I->width - 1)) +
		newsizex * (int) ((newsizey - 1) * (double) y /
				  (I->height - 1));
	      shrinknumbers[i]++;
  ...
  for (i = 0; i < newsizey * newsizex; i++)
	...
	Imagedest->data[i] =
	  (((int) (shrinkred[i] / shrinknumbers[i]) >> 14) << 4) |
...
void
XpmShapeToFit_fast (int newsizex, int newsizey, XpmImage * I,
		    XpmImage * Imagedest)
{
  int k, x, y;
  ...
  for (k = 0; k < I->ncolors; k++)
	...
	(Imagedest->colorTable)[k].string =
	((I->colorTable)[k].
	 string ? strdup ((I->colorTable)[k].string) : NULL);
      (Imagedest->colorTable)[k].symbolic =
	((I->colorTable)[k].
	 symbolic ? strdup ((I->colorTable)[k].symbolic) : NULL);
      (Imagedest->colorTable)[k].m_color =
	((I->colorTable)[k].
	 m_color ? strdup ((I->colorTable)[k].m_color) : NULL);
      (Imagedest->colorTable)[k].g4_color =
	((I->colorTable)[k].
	 g4_color ? strdup ((I->colorTable)[k].g4_color) : NULL);
      (Imagedest->colorTable)[k].g_color =
	((I->colorTable)[k].
	 g_color ? strdup ((I->colorTable)[k].g_color) : NULL);
      (Imagedest->colorTable)[k].c_color =
	((I->colorTable)[k].
	 c_color ? strdup ((I->colorTable)[k].c_color) : NULL);
    }
  ...
  for (y = 0; y < newsizey; y++)
    for (x = 0; x < newsizex; x++)
      {
	Imagedest->data[k] =
	  I->data[ROUND ((double) x * step_x) +
		  ((ROUND ((double) y * step_y)) * (I->width))];
	k++;
      }
}
...
MyCreatePixmapFromXpmImage (XpmImage * Img, Pixmap * pixmap, Pixmap * mask)
{
  ...
  XGetWindowAttributes (GDK_DISPLAY (),
			GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()), &root_attr);
...
GdkPixmap *
MyCreateGdkPixmapFromFile (char *filename, GtkWidget * widget,
			   GdkBitmap ** mask, gboolean fit)
{
#ifdef HAVE_GDK_IMLIB
  GdkImlibImage *im;
#else
  ...
  if (source)
    {
      ...
      h = requisition.height - (GTK_CONTAINER (widget)->border_width + 2) * 2;
      if ((fit) && (w > 0) && (h > 0) &&
	  ((gdk_pixbuf_get_width (source) > w)
	   || gdk_pixbuf_get_height (source) > h))
	destination =
	  gdk_pixbuf_scale_simple (source, w, h, GDK_INTERP_BILINEAR);
      else
	destination =
	  gdk_pixbuf_scale_simple (source, gdk_pixbuf_get_width (source),
				   gdk_pixbuf_get_height (source),
				   GDK_INTERP_BILINEAR);
      gdk_pixbuf_render_pixmap_and_mask (destination, &pixmap, mask,
  ...
  if (result == 0)
      ...
      else
	{
      ...
      pixmap =
	gdk_pixmap_colormap_create_from_xpm_d (widget->window, colormap, mask,
					       NULL, (gchar **) pix_data);
      XpmFree (pix_data);
      XpmFreeXpmImage (&I);
    }
...
BuildXpmGradient (int r1, int g1, int b1, int r2, int g2, int b2)
{
  ...
  GradientXpmImage (10, gdk_screen_height (), &I, r1, g1, b1, r2, g2, b2,
		    gradient_steps);
  status = MyCreatePixmapFromXpmImage (&I, &rootXpm, &mask);
  if (status == XpmSuccess)
    {
      XSetWindowBackgroundPixmap (GDK_DISPLAY (),
				  GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()),
				  rootXpm);
  ...
  if ((DEFAULT_DEPTH > 8) && (gradient))
	...
	prop = XInternAtom (GDK_DISPLAY (), "_XROOTPMAP_ID", False);
      ...
      XGetWindowProperty (GDK_DISPLAY (),
			  GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()), prop, 0L,
      ...
      if ((type == XA_PIXMAP) && (format == 32) && (length == 1))
	{
      ...
      XDeleteProperty (GDK_DISPLAY (),
		       GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()), prop);
      XSetWindowBackground (GDK_DISPLAY (),
			    GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()),
			    get_pixel_from_palette (pal, ci));
      XClearWindow (GDK_DISPLAY (), GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()));
      XUngrabServer (GDK_DISPLAY ());
      XFlush (GDK_DISPLAY ());
    }
