...
initialize_pager (void)
{
  ...
  if (Scr.d_depth < 2)
	...
	XCreatePixmapFromBitmapData (dpy, Scr.Root, l_g_bits, l_g_width,
				     l_g_height, fore_pix, back_pix,
				     Scr.d_depth);
      Scr.sticky_gray_pixmap =
	XCreatePixmapFromBitmapData (dpy, Scr.Root, s_g_bits, s_g_width,
				     s_g_height, fore_pix, back_pix,
				     Scr.d_depth);
    }
  ...
  if (window_h > 0)
    {
      Scr.VScale =
	Rows * Scr.MyDisplayHeight / (window_h + 2 - Rows * (label_h - 1));
    }
  ...
  for (i = 0; i < ndesks; i++)
    {
      ...
      h = window_h - label_h;
      Desks[i].CPagerWin =
	XCreateWindow (dpy, Desks[i].w, -1000, -1000, w, h, 0, CopyFromParent,
  ...
  for (k = 0; k < Rows; k++)
      ...
      for (j = 0; j < Columns; j++)
	{
	  ...
	  if (i < ndesks)
		...
		XMoveResizeWindow (dpy, Desks[i].CPagerWin, -1000, -100, w,
				   h);
