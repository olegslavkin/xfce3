		       ...
		       void *client)
{
  ...
  if ((data->length == 0) || (data->length > DLG_MAX) ||
...
dlg_new (char *labelval, char *defval, void *data, int type)
{
  GtkWidget *ok = NULL,
    *cancel = NULL,
    *all = NULL,
    *skip = NULL, *close = NULL, *icon = NULL, *combo =
    NULL, *label, *box, *button_box;
  char title[DLG_MAX];
  char *longlabel = NULL;
  ...
  gtk_box_pack_start (GTK_BOX (GTK_DIALOG (dl.top)->vbox), box, TRUE,
		      TRUE, 0);
  ...
  if (type & DLG_ENTRY_VIEW)
  ...
  else if (type & DLG_ENTRY_EDIT)
    {
      ...
      gtk_drag_dest_set (dl.entry, GTK_DEST_DEFAULT_ALL,
			 target_table, NUM_TARGETS, GDK_ACTION_COPY);
      gtk_signal_connect (GTK_OBJECT (dl.entry),
			  "drag_data_received",
			  GTK_SIGNAL_FUNC (on_drag_data_received), NULL);
    }
  else if (type & DLG_COMBO)
    {
      ...
      gtk_drag_dest_set (dl.entry, GTK_DEST_DEFAULT_ALL,
			 target_table, NUM_TARGETS, GDK_ACTION_COPY);
      gtk_signal_connect (GTK_OBJECT (dl.entry), "activate",
			  GTK_SIGNAL_FUNC (on_ok),
			  (gpointer) ((long) DLG_RC_OK));
      gtk_signal_connect (GTK_OBJECT (dl.entry),
			  "drag_data_received",
  ...
  if (dl.entry)
      ...
      else
	{
	  ...
	  gtk_signal_connect_object (GTK_OBJECT (dl.entry),
				     "activate",
				     GTK_SIGNAL_FUNC
