...
AddWindow (Window w)
{
    ...
    if (ShapesSupported)
    {
        ...
        XShapeQueryExtents (dpy, w,
                            &boundingShaped, &xws, &yws, &wws, &hws,
                            &clipShaped, &xbs, &ybs, &wbs, &hbs);
        tmp_win->wShaped = boundingShaped;
    }

    tflag = LookInStyleList (tmp_win->name, &tmp_win->class, &value,
                             &Desk, &border_width, &layer, &forecolor, &backcolor,
                             &tmp_win->buttons);
    tmp_win->layer = layer;
    ...
    if (XGetCommand (dpy, w, &client_argv, &client_argc))
    {
        ...
        status =
            XrmGetResource (db, "xfwm.desk", "Xfwm.Desk", &str_type, &rm_value);
    ...
    if ((XGetWindowProperty (dpy, w, _XA_XFWM_FLAGS, 0L, 1L, True,
                             ...
                             &bytes_remain, &prop)) == Success)
    {
        ...
        {
            tmp_win->flags |=
                (*(unsigned long *) prop) & (STICKY | ICON_MOVED | SHADED | RECAPTURE);
            XFree (prop);
        }
    }

    if ((XGetWindowProperty (dpy, w, _XA_XFWM_ICONPOS_X, 0L, 1L, True,
    ...
    if ((XGetWindowProperty (dpy, w, _XA_XFWM_ICONPOS_Y, 0L, 1L, True,
    ...
    tmp_win->Parent =
        XCreateWindow (dpy, tmp_win->frame,
                       ...
                       tmp_win->frame_width - 2 * (tmp_win->boundary_width + tmp_win->bw),
                       tmp_win->frame_height - 2 * (tmp_win->boundary_width + tmp_win->bw) - tmp_win->title_height,
    ...
    if (tmp_win->flags & TITLE)
    {
        ...
        tmp_win->title_w =
            XCreateWindow (dpy, tmp_win->frame, tmp_win->title_x,
            ...
            if ((i < Scr.nr_left_buttons) && (tmp_win->left_w[i] > 0))
            {
                tmp_win->left_w[i] =
                    XCreateWindow (dpy, tmp_win->frame, tmp_win->title_height * i,
                                   ...
                                   InputOutput, CopyFromParent, valuemask | CWBackPixel,
                                   &attributes);
            }
            else
            ...
            if ((i < Scr.nr_right_buttons) && (tmp_win->right_w[i] > 0))
            {
                tmp_win->right_w[i] =
                    XCreateWindow (dpy, tmp_win->frame,
                                   ...
                                   0, CopyFromParent, InputOutput,
                                   CopyFromParent, valuemask | CWBackPixel, &attributes);
            }
            else
    ...
    if (tmp_win->flags & TITLE)
    {
        ...
        for (i = 0; i < Scr.nr_left_buttons; i++)
            XSaveContext (dpy, tmp_win->left_w[i], XfwmContext,
                          (caddr_t) tmp_win);
    ...
    if (tmp_win->flags & BORDER)
    {
        for (i = 0; i < 4; i++)
        {
            XSaveContext (dpy, tmp_win->sides[i], XfwmContext,
                          (caddr_t) tmp_win);
            XSaveContext (dpy, tmp_win->corners[i], XfwmContext,
                          (caddr_t) tmp_win);
        }
    }

    MyXGrabButton (dpy, AnyButton, 0, tmp_win->frame, True,
                   ButtonPressMask, GrabModeSync, GrabModeAsync, None, None);
    MyXGrabButton (dpy, AnyButton, AnyModifier, tmp_win->frame, True,
                   ButtonPressMask, GrabModeSync, GrabModeAsync, None, None);

    BroadcastConfig (XFCE_M_ADD_WINDOW, tmp_win);

    BroadcastName (XFCE_M_WINDOW_NAME, w, tmp_win->frame,
                   (unsigned long) tmp_win, tmp_win->name);
    BroadcastName (XFCE_M_ICON_NAME, w, tmp_win->frame,
                   (unsigned long) tmp_win, tmp_win->icon_name);
    if (tmp_win->icon_bitmap_file != NULL &&
            tmp_win->icon_bitmap_file != Scr.DefaultIcon)
        BroadcastName (XFCE_M_ICON_FILE, w, tmp_win->frame,
                       (unsigned long) tmp_win, tmp_win->icon_bitmap_file);
    BroadcastName (XFCE_M_RES_CLASS, w, tmp_win->frame,
                   (unsigned long) tmp_win, tmp_win->class.res_class);
    BroadcastName (XFCE_M_RES_NAME, w, tmp_win->frame,
...
GrabButtons (XfwmWindow * tmp_win)
{
    ...
    while (MouseEntry != (Binding *) 0)
    {
                ...
                && (MouseEntry->IsMouse == 1))
        {
            ...
            {
                MyXGrabButton (dpy, MouseEntry->Button_Key,
            ...
            {
                MyXGrabButton (dpy, AnyButton, MouseEntry->Modifier,
                               tmp_win->w, True,
                               ButtonPressMask | ButtonReleaseMask,
                               GrabModeAsync, GrabModeAsync, None, None);
            }
...
GrabKeys (XfwmWindow * tmp_win)
{
    ...
    for (tmp = Scr.AllBindings; tmp != NULL; tmp = tmp->NextBinding)
    {
        if ((tmp->Context & (C_WINDOW | C_TITLE | C_RALL | C_LALL | C_SIDEBAR))
                && (tmp->IsMouse == 0))
        {
            MyXGrabKey (dpy, tmp->Button_Key, tmp->Modifier, tmp_win->frame,
...
Bool GetWindowState (XfwmWindow * tmp, unsigned long *state)
{
    Atom atype;
    int aformat;
    unsigned long nitems, bytes_remain;
    ...
    if ((XGetWindowProperty (dpy, tmp->w, _XA_WIN_STATE, 0L, 1L, False,
                             XA_CARDINAL, &atype, &aformat, &nitems, &bytes_remain,
                             &prop)) == Success)
    {
        if (prop != NULL)
        {
...
void SetWindowState (XfwmWindow * tmp)
{
    Atom atype;
    int aformat;
    unsigned long nitems, bytes_remain;
                             ...
                             &prop)) == Success)
    {
	    ...
	    if (newstate != oldstate)
	      {
                XChangeProperty (dpy, (Window) tmp->w, _XA_WIN_STATE, XA_CARDINAL,
		                 32, PropModeReplace, (unsigned char *) &newstate, 1);
	      }
        }
    }
}
