...
SaveWindowStates (FILE * f)
{
    ...
    for (tmp_win = Scr.XfwmRoot.next; tmp_win; tmp_win = tmp_win->next)
    {
        ...
        if ((tmp_win->flags & TRANSIENT)
                && (tmp_win->transientfor != tmp_win->w))
            continue;
        ...
        if (GetWindowCommand (tmp_win, &wm_command, &wm_command_count)
                && (wm_command_count > 0))
        {
        ...
        fprintf (f, "  [GEOMETRY] %i %i %i %i %i %i\n",
                 tmp_win->orig_x,
                 tmp_win->orig_y,
                 tmp_win->orig_wd - 2 * (tmp_win->boundary_width + tmp_win->bw),
                 tmp_win->orig_ht - 2 * (tmp_win->boundary_width + tmp_win->bw) -	tmp_win->title_height,
                 tmp_win->icon_x_loc,
                 tmp_win->icon_y_loc);
        fprintf (f, "  [DESK] %i\n",
                 ((tmp_win->flags & (STICKY)) ? 0 : tmp_win->Desk));
        fprintf (f, "  [FLAGS] %li\n",
                 (tmp_win->flags & (STICKY | ICONIFIED | ICON_MOVED | SHADED |
                                    WM_NAME_CHANGED)));
    }
    return 1;
}
    ...
    if (xstreq (client_id, m->client_id))
    {
        ...
        else
        {
            ...
            if (xstreq (w->class.res_name, m->res_name) &&
                    ...
                    ((w->flags & WM_NAME_CHANGED) || (m->flags & WM_NAME_CHANGED)
...
MatchWinToSM (XfwmWindow * tmp_win)
{
    ...
    for (i = 0; i < num_match; i++)
    {
        if (!matches[i].used && matchWin (tmp_win, &matches[i]))
        {
            ...
            tmp_win->flags |=
                (matches[i].flags & (STICKY | ICON_MOVED | SHADED));
...
builtin_session_startup (void)
{
    ...
    for (i = 0; i < num_match; i++)
    {
        if (matches[i].wm_command_count)
        {
            ...
            if ((matches[i].wm_command_count > 6)
                    && (!mystrncasecmp (matches[i].wm_command[6], "XFwm", 4)))
                continue;
            ...
            found = False;
            for (j = 0; j < i; j++)
                if ((matches[i].client_leader)
                        && (matches[j].client_leader == matches[i].client_leader))
