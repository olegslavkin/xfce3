    ...
    while (temp)
    {
        ...
        if (temp->IsMouse == mouse_binding)
        {
                    ...
                    && (temp->Context == contexts) && (temp->Modifier == mods))
            {
                ...
                if ((temp->Action != NULL) && (temp->Context & C_WINDOW)
                        && (temp->IsMouse == 1))
                {
                    for (t = Scr.XfwmRoot.next; t != NULL; t = t->next)
                    {
                        if (temp->Button_Key > 0)
                            MyXUngrabButton (dpy, temp->Button_Key,
                                             temp->Modifier, t->w);
                        else
                            MyXUngrabButton (dpy, AnyButton, temp->Modifier,
                                             t->w);
                    }
                }
 ...
 ****************************************************************************/
void
ParseMouseEntry (XEvent * eventp, Window w, XfwmWindow * tmp_win,
                 unsigned long junk, char *tline, int *Module)
{
    ...
    if ((n1 != 1) || (n2 != 1) || (n3 != 1))
    {
        xfwm_msg (ERR, "ParseMouseEntry",
                  "Mouse binding: Syntax error in line %s", tline);
        return;
    }
 ...
 ****************************************************************************/
void
ParseKeyEntry (XEvent * eventp, Window w, XfwmWindow * tmp_win,
               unsigned long junk, char *tline, int *Module)
{
