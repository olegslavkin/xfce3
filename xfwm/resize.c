 ...
 ****************************************************************************/
void
resize_window (XEvent * eventp, Window w, XfwmWindow * tmp_win,
               unsigned long context, char *action, int *Module)
{
    ...
    if (n == 2)
    {
        ...
        SetupFrame (tmp_win, tmp_win->frame_x,
                    tmp_win->frame_y, dragWidth, dragHeight, FALSE, TRUE);
    ...
    if (menuFromFrameOrWindowOrTitlebar)
    {
        XWarpPointer (dpy, Scr.Root, Scr.Root,
                      ...
                      my_min(MyDisplayMaxX(Stashed_X,Stashed_Y) - 1, Stashed_X),
                      my_min(MyDisplayMaxY(Stashed_X,Stashed_Y) - 1, Stashed_Y));
    }
    ...
    while (!finished)
    {
        ...
        case MotionNotify:
            ...
            break;
        case DestroyNotify:
            if ((Event.xdestroywindow.window == tmp_win->frame) ||
                    (Event.xdestroywindow.window == tmp_win->w))
            {
                finished = TRUE;
                done = TRUE;
                window_deleted = True;
    ...
    if (!window_deleted)
    {
        ...
        {
            ConstrainSize (tmp_win, &dragWidth, &dragHeight);
            SetupFrame (tmp_win, dragx, dragy, dragWidth,
                        ((tmp_win->flags & SHADED) ? initHeight : dragHeight),
                        FALSE, TRUE);
        }
        else
            SetupFrame (tmp_win, origx, origy, origWidth,
                        ((tmp_win->flags & SHADED) ? initHeight : origHeight),
                        FALSE, TRUE);
    }
    ...
    while ((!window_deleted) && XPending (dpy))
    {
        ...
        if ((Event.type == ConfigureRequest)
                && (Event.xconfigurerequest.window == tmp_win->w))
            Event.xconfigurerequest.value_mask &=
                ~(CWX | CWY | CWWidth | CWHeight | CWBorderWidth);
        else if ((Event.type == DestroyNotify)
                 && (Event.xdestroywindow.window == tmp_win->frame))
            window_deleted = True;
        DispatchEvent ();
    }
...
DoResize (int x_root, int y_root, XfwmWindow * tmp_win)
{
    ...
    if ((y_root <= origy)
            || ((ymotion == 1) && (y_root < origy + origHeight - 1)))
    {
    ...
    else if ((x_root >= origx + origWidth - 1) ||
...
MoveOutline (XfwmWindow * tmp_win, int x, int y, int width, int height)
{
    ...
    {
        XDrawRectangle (dpy, Scr.Root, Scr.DrawGC, lastx, lasty, lastWidth,
                        lastHeight);
        if (tmp_win)
...
DisplayPosition (XfwmWindow * tmp_win, int x, int y, int w, int h)
{
    ...
    XDrawString (dpy, Scr.Root, Scr.HintsGC,
                 x + (w - offsetx) / 2, y + 5 + Scr.StdFont.height,
                 strxy, strlen (strxy));
    XDrawString (dpy, Scr.Root, Scr.HintsGC,
                 x + (w - offsetw) / 2, y + h + 5 + Scr.StdFont.height,
                 strw, strlen (strw));
    XDrawString (dpy, Scr.Root, Scr.HintsGC,
                 x + w + 5, y + (h + Scr.StdFont.height) / 2,
                 strh, strlen (strh));
    if (h > 3 * Scr.StdFont.height)
        XDrawString (dpy, Scr.Root, Scr.HintsGC,
                     x + (w - offsetd) / 2, y + (h + Scr.StdFont.height) / 2,
                     strd, strlen (strd));
    points[0].x = x + 15;
    points[0].y = y + h + 5;
    points[1].x = x;
    ...
    points[2].x = x + 15;
    points[2].y = y + h + 15;
    XFillPolygon (dpy, Scr.Root, Scr.HintsGC, points, 3, Convex,
                  CoordModeOrigin);
    points[0].x = x + w - 15;
    points[0].y = y + h + 5;
    points[1].x = x + w;
    ...
    XFillPolygon (dpy, Scr.Root, Scr.HintsGC, points, 3, Convex,
                  CoordModeOrigin);
    XDrawLine (dpy, Scr.Root, Scr.HintsGC, x + 15, y + h + 10,
               x + (w - offsetw) / 2 - 5, y + h + 10);
    XDrawLine (dpy, Scr.Root, Scr.HintsGC, x + (w + offsetw) / 2 + 5,
               y + h + 10, x + w - 15, y + h + 10);
    XDrawLine (dpy, Scr.Root, Scr.HintsGC, x + w + 10, y + 15, x + w + 10,
               y + (h - Scr.StdFont.height) / 2 - 5);
    XDrawLine (dpy, Scr.Root, Scr.HintsGC, x + w + 10,
               y + (h + Scr.StdFont.height) / 2 + 5, x + w + 10, y + h - 15);
}
