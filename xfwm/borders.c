        ...
        for (i = 0; i < 4; i++)
        {
            ...
            if (flush_expose (t->corners[i]) || (expose_win == t->corners[i]) || (expose_win == None) || NewColor)
            {
                ...
                RelieveWindow (t, t->corners[i], 0, 0, t->corner_width,
                               t->corner_width, rgc, sgc, corners[i]);
    ...
    if (Resized)
    {
        ...
        if (tmp_win->flags & BORDER)
        {
            tmp_win->corner_width =
                GetDecor (tmp_win,
                          TitleHeight) + tmp_win->boundary_width;
#ifdef OLD_STYLE
...
if (!(tmp_win->flags & TITLE))
    ...
    if (tmp_win->flags & SHADED)
        XMoveResizeWindow (dpy, tmp_win->Parent, cx, cy - tmp_win->attr.height - tmp_win->title_height - tmp_win->boundary_width - 2 * tmp_win->bw,
...
SetShape (XfwmWindow * tmp_win, int w)
{
    if ((ShapesSupported) && (tmp_win->wShaped))
    {
        ...
        XShapeCombineShape (dpy, tmp_win->frame, ShapeBounding,
                            tmp_win->boundary_width + tmp_win->bw,
