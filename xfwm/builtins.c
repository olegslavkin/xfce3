    ...
    if (Scr.Options & AnimateWin)
    {
        ...
        for (j = 0; j < 11; j++)
        {
            MoveOutline (0, o_x + j * delta_x / 12,
                         o_y + j * delta_y / 12,
                         o_w + j * delta_w / 12, o_h + j * delta_h / 12);
            XFlush (dpy);
            my_sleep (2);
        }
...
void
ShowMeMouse (XEvent * eventp, Window junk, XfwmWindow * tmp_win,
...
WarpOn (XfwmWindow * t, int warp_x, int x_unit, int warp_y, int y_unit)
{
    ...
    else
    {
        ...
        if (y_unit != MyDisplayHeight (warp_x, warp_y))
            ...
            y =
                t->frame_y + 2 +
                (((t->flags & SHADED) ? t->shade_height : t->frame_height) -
                 4) * warp_y / 100;
    }
    if (warp_x >= 0 && warp_y >= 0)
    {
        XWarpPointer (dpy, Scr.Root, Scr.Root,
                      0, 0, Scr.MyDisplayWidth, Scr.MyDisplayHeight,
                      my_min(MyDisplayMaxX(x,y) - 1, x),
                      my_min(MyDisplayMaxY(x,y) - 1, y));
    }
    RaiseWindow (t);
...
Shade (XfwmWindow * tmp_win)
{
    ...
    Broadcast (XFCE_M_SHADE, 3, tmp_win->w, tmp_win->frame,
...
Unshade (XfwmWindow * tmp_win)
{
    ...
    SetBorder (tmp_win, (Scr.Hilite == tmp_win), True, True, None);
    Broadcast (XFCE_M_UNSHADE, 3, tmp_win->w, tmp_win->frame,
               (unsigned long) tmp_win, 0, 0, 0, 0);
}
          ...
          unsigned long context, char *action, int *Module)
{
    ...
    if (tmp_win->flags & MAXIMIZED)
    {
        ...
        if (!(tmp_win->flags & ICONIFIED))
        {
            Broadcast (XFCE_M_DEMAXIMIZE, 3, tmp_win->w, tmp_win->frame,
                       (unsigned long) tmp_win, 0, 0, 0, 0);
            if (tmp_win->flags & SHADED)
                Animate (tmp_win->frame_x,
                         tmp_win->frame_y,
                ...
                Animate (tmp_win->frame_x,
                         ...
                         tmp_win->frame_height,
                         tmp_win->orig_x,
                         tmp_win->orig_y, tmp_win->orig_wd, tmp_win->orig_ht);
        }
...
void
Nop_func (XEvent * eventp, Window w, XfwmWindow * tmp_win,
          unsigned long context, char *action, int *Module)
{
}

void
movecursor (XEvent * eventp, Window w, XfwmWindow * tmp_win,
            unsigned long context, char *action, int *Module)
{
    int x, y;
    int val1, val2, val1_unit, val2_unit, n;
    ...
    XWarpPointer (dpy, Scr.Root, Scr.Root,
                  0, 0, Scr.MyDisplayWidth, Scr.MyDisplayHeight,
                  my_min(MyDisplayMaxX(warpx,warpy) - 1, warpx),
                  my_min(MyDisplayMaxY(warpx,warpy) - 1, warpy));
}
...
void
Iconify_all (XEvent * eventp, Window w, XfwmWindow * tmp_win,
             unsigned long context, char *action, int *Module)
{
    XfwmWindowList *t;
    unsigned int opt;
    ...
    while (t)
    {
        if (!((t->win)->flags & ICONIFIED) &&
                !((t->win)->flags & TRANSIENT) &&
                ((t->win)->flags & TITLE) &&
                ((t->win)->Desk == Scr.CurrentDesk))
        {
                      ...
                      unsigned long context, char *action, int *Module)
{
    ...
    w = XCreateWindow (dpy,
                       (context ==
                        C_ICON) ? (tmp_win->icon_w) : (tmp_win->frame), 0, 0,
                       (unsigned int) Scr.MyDisplayWidth,
                ...
                unsigned long context, char *action, int *Module)
{
    ...
    if     ((mystrncasecmp (mode, "On", 2) == 0) ||
            (mystrncasecmp (mode, "Y", 1) == 0) ||
        ...
        unsigned long context, char *action, int *Module)
{
    ...
    gcm =
        GCFunction | GCLineWidth | GCForeground | GCSubwindowMode | GCCapStyle;
    gcv.function = GXxor;
    gcv.line_width = 1;
    gcv.cap_style = CapProjecting;
    gcv.foreground =
        (val1) ? (val1) : (BlackPixel (dpy, Scr.screen) ^
             ...
             unsigned long context, char *action, int *Module)
{
    ...
    {				/* Temporarily, set FontStruct for calculating. */
        ...
        int font_num = 0, i;
        font_num =
            XFontsOfFontSet (Scr.StdFont.fontset, &fs_list, &fontset_namelist);
              ...
              unsigned long context, char *action, int *Module)
{
    ...
    {				/* Temporarily, set FontStruct for calculating. */
        ...
        int font_num = 0, i;
        font_num =
            XFontsOfFontSet (Scr.IconFont.fontset, &fs_list, &fontset_namelist);
    ...
    {
        Scr.IconFont.height =
            Scr.IconFont.xftfont->ascent + Scr.IconFont.xftfont->descent;
                ...
                unsigned long context, char *action, int *Module)
{
    ...
    {			/* Temporarily, set FontStruct for calculating. */
        ...
        int font_num = 0, i;
        font_num =
            XFontsOfFontSet (fl->WindowFont.fontset, &fs_list,
                             &fontset_namelist);
...
Circulate (char *action, int Direction, char **restofline)
{
    ...
    char *t, *tstart, *name = NULL, *expression, *condition, *prev_condition =
                                  NULL;
    char *orig_expr;
    ...
    while ((pass < 3) && (found == NULL))
    {
        while ((fw != NULL) && (found == NULL) && (fw != &Scr.XfwmRoot))
        {
            ...
            if ((onFlags & fw->flags) == onFlags &&
                    ...
                    (!needsCurrentPage || (fw->frame_x < MyDisplayMaxX (fw->frame_x, fw->frame_y) &&
                                           fw->frame_y < MyDisplayMaxY (fw->frame_x, fw->frame_y) &&
                                           fw->frame_x + fw->frame_width > MyDisplayX (fw->frame_x, fw->frame_y) &&
                                           fw->frame_y + fw->frame_height > MyDisplayY (fw->frame_x, fw->frame_y))) &&
                    ((!needsName ||
...
RedrawSwitchWindow (Window w, char *name, int width, int height)
{
	...
	XftColor color_fg;
        ...
        xftdraw = XftDrawCreate(dpy, (Drawable) w, 
	...
	color_fg.pixel = TextColor;
        ...
        XftDrawString8 (xftdraw, &color_fg, Scr.StdFont.xftfont, 10, (height + Scr.StdFont.y) / 2, name, strlen(name));
...
void
SwitchFunc (XEvent * eventp, Window junk, XfwmWindow * tmp_win,
            unsigned long context, char *action, int *Module)
{
    XfwmWindow *t, *tp;
    Bool finished = FALSE;
    ...
    XQueryPointer (dpy, Scr.Root, &JunkRoot, &JunkChild,
                   &mousex, &mousey, &JunkX, &JunkY, &JunkMask);
    ...
    while (!finished)
    {
        if (namechanged)
        {
            ...
            else if ((enable_xft) && (Scr.StdFont.xftfont))
            {
                ...
                XftTextExtents8 (dpy, Scr.StdFont.xftfont, t->name, strlen(t->name), &extents);
                width = extents.xOff;
            }
#endif
            else
            ...
            else
            {
                ...
                taskw = XCreateWindow (dpy, Scr.Root, wx, wy, width, height, 0,
        ...
        case KeyPress:
            ...
            if (keysym == XK_Tab)
            {
                ...
                while ((t->flags & CirculateSkip) ||
                        ((t->flags & CirculateSkipIcon) && (t->flags & ICONIFIED)));
            }
              ...
              unsigned long context, char *action, int *Module)
{
    ...
    else if ((mystrncasecmp (style, "Mouse", 5) == 0) ||
             (mystrncasecmp (style, "Follo", 5) == 0))
    {
        if (Scr.Options & ClickToFocus)
            Scr.Options &= ~ClickToFocus;
    }
              ...
              unsigned long context, char *action, int *Module)
{
    ...
    else if ((mystrncasecmp (style, "Of", 2) == 0) ||
             (mystrncasecmp (style, "N", 1) == 0) ||
                   ...
                   unsigned long context, char *action, int *Module)
{
    ...
    if (n != 1)
    {
        xfwm_msg (ERR, "SetAutoRaiseDelay",
                  "AutoRaiseDelay requires 1 argument");
        return;
    }
            ...
            unsigned long context, char *action, int *Module)
{
    ...
    if ((mystrncasecmp (style, "T", 1) == 0) ||
            (mystrncasecmp (style, "0", 1) == 0))
        Scr.iconbox = 0;
    else if ((mystrncasecmp (style, "L", 1) == 0) ||
             (mystrncasecmp (style, "1", 1) == 0))
        Scr.iconbox = 1;
    else if ((mystrncasecmp (style, "B", 1) == 0) ||
             (mystrncasecmp (style, "2", 1) == 0))
        Scr.iconbox = 2;
    else if ((mystrncasecmp (style, "R", 1) == 0) ||
    ...
    for (n = 0; n <= NBSCREENS; n++)
        ...
        {
            if (((t->win)->flags & ICONIFIED) && ((t->win)->Desk == n)
                    && !((t->win)->flags & ICON_MOVED))
            {
...
void
wait_session (XEvent * eventp, Window w, XfwmWindow * tmp_win,
              unsigned long context, char *action, int *Module)
{
    int val1;
    int val1_unit, n;
...
void
SetSessionMgt (XEvent * eventp, Window junk, XfwmWindow * tmp_win,
               unsigned long context, char *action, int *Module)
{
    char *style = NULL;
    ...
    {
        xfwm_msg (ERR, "SetSessionMgt",
                  "Unknown parameter, using session management On");
        Scr.Options |= SessionMgt;
    }
    free (style);
}
...
void
SetIconGrid (XEvent * eventp, Window w, XfwmWindow * tmp_win,
             unsigned long context, char *action, int *Module)
{
    int val1;
    int val1_unit, n;
...
void
SetIconSpacing (XEvent * eventp, Window w, XfwmWindow * tmp_win,
                unsigned long context, char *action, int *Module)
{
    int val1;
    int val1_unit, n;
...
void
engine_func (XEvent * eventp, Window w, XfwmWindow * tmp_win,
             unsigned long context, char *action, int *Module)
{
    char *style = NULL;
    XfwmWindowList *t;
    ...
    {
        xfwm_msg (ERR, "Engine",
                  "Unknown parameter, using Xfce engine");
        Scr.engine = XFCE_ENGINE;
    }
...
void
show_buttons (XEvent * eventp, Window w, XfwmWindow * tmp_win,
              unsigned long context, char *action, int *Module)
{
    XfwmWindowList *t;
    int val1;
...
void
SetClickRaise (XEvent * eventp, Window junk, XfwmWindow * tmp_win,
               unsigned long context, char *action, int *Module)
{
    char *style = NULL;
    ...
    {
        xfwm_msg (ERR, "SetClickRaise",
                  "Unknown parameter, using ClickRaise Off");
        Scr.Options &= ~ClickRaise;
    }
    free (style);
}
...
void
SetForceFocus (XEvent * eventp, Window junk, XfwmWindow * tmp_win,
               unsigned long context, char *action, int *Module)
{
    char *style = NULL;
    ...
    {
        xfwm_msg (ERR, "SetForceFocus",
                  "Unknown parameter, using ForceFocus Off");
        Scr.Options &= ~ForceFocus;
    }
    free (style);
}
...
void
SetMapFocus (XEvent * eventp, Window junk, XfwmWindow * tmp_win,
             unsigned long context, char *action, int *Module)
{
    char *style = NULL;
    ...
    {
        xfwm_msg (ERR, "SetMapFocus",
                  "Unknown parameter, using MapFocus On");
        Scr.Options |= MapFocus;
    }
    free (style);
}
...
void
SetMargin (XEvent * eventp, Window junk, XfwmWindow * tmp_win,
            unsigned long context, char *action, int *Module)
{
    int val1;
    int val1_unit, n;
    ...
    action = GetNextToken (action, &side);
    if      ((mystrncasecmp (side, "T", 1) == 0) ||
             (mystrncasecmp (side, "0", 1) == 0))
        where = 0;
    else if ((mystrncasecmp (side, "L", 1) == 0) ||
             (mystrncasecmp (side, "1", 1) == 0))
        where = 1;
    else if ((mystrncasecmp (side, "B", 1) == 0) ||
             (mystrncasecmp (side, "2", 1) == 0))
        where = 2;
    else if ((mystrncasecmp (side, "R", 1) == 0) ||
             (mystrncasecmp (side, "3", 1) == 0))
        where = 3;
    else
    {
