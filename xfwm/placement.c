...
unsigned long
overlap(int x0, int y0, int x1, int y1, int tx0,
    ...
    for (test_y = MyDisplayY (px, py) + Scr.Margin[0]; test_y < ymax; test_y += 5)
    {
        for (test_x = MyDisplayX (px, py) + Scr.Margin[1]; test_x < xmax; test_x += 5)
        {
            ...
            while ((t != NULL) && ((count_overlaps < best_overlaps) || first))
            {
                if ((t == tmp_win) ||
                        ((t->Desk) != (tmp_win->Desk)) ||
                        ((t->layer) != (tmp_win->layer)) ||
                        (t->flags & (ICONIFIED)))
                {
                    t = t->next;
                    continue;
                }
                count_overlaps += overlap (test_x,
                                           test_y,
                                           test_x + bw,
                                           test_y + bh,
                                           t->frame_x,
                                           t->frame_y,
                                           t->frame_x + ((t->flags & SHADED) ? t->shade_width : t->frame_width),
                                           t->frame_y + ((t->flags & SHADED) ? t->shade_height : t->frame_height));
                t = t->next;
            }
            if (count_overlaps == 0)
            {
...
#if defined(HAVE_X11_EXTENSIONS_XINERAMA_H) || defined(EMULATE_XINERAMA)
        if ((!(tmp_win->flags & RECAPTURE)) &&
                (xinerama_heads != 0) &&
                (xinerama_infos != NULL) &&
                (enable_xinerama))
        {
            /* Translate coodinates to center on physical screen */
            if ((abs(tmp_win->attr.x - ((Scr.MyDisplayWidth - tmp_win->attr.width) / 2)) < 20) &&
                    (abs(tmp_win->attr.y - ((Scr.MyDisplayHeight - tmp_win->attr.height) / 2)) < 20))
            {
                /* We consider that the windows is centered on screen,
                 * Thus, will move it so its center on the current 
                 * physical screen
                 */
                tmp_win->attr.x = MyDisplayX (xl, yt) +
                                  (MyDisplayWidth (xl, yt) - tmp_win->attr.width) / 2;
                tmp_win->attr.y = MyDisplayY (xl, yt) +
                                  (MyDisplayHeight (xl, yt) - tmp_win->attr.height) / 2;
            }
...
SnapXY (int *x, int *y, XfwmWindow * skip)
{
    ...
    changed = snapxy (&nx, &ny, MyDisplayX (*x, *y),
                      MyDisplayY (*x, *y),
                      MyDisplayMaxX (*x, *y),
                      MyDisplayMaxY (*x, *y));
