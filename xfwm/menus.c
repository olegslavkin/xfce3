...
do_menu (MenuRoot * menu, int style)
{
    ...
    if (menu_on)
    {
        ...
        if (ActiveMenu)
        {
            if ((Stashed_X + (ActiveMenu->width >> 1) + menu->width) <
                    MyDisplayMaxX (Stashed_X, Stashed_Y))
...
FindEntry (void)
{
    ...
    /* end a sub-menu */
    if (XFindContext (dpy, Child, MenuContext, (caddr_t *) &actual_mr) ==
            XCNOENT)
    {
        return retval;
    }
...
menuShortcuts (XEvent * ev)
{
    ...
    if (((kpressed[0] >= 'a') && (kpressed[0] <= 'z')) ||	/* Only consider alphabetic */
        ((kpressed[0] >= 'A') && (kpressed[0] <= 'Z')) ||
        ((kpressed[0] >= '0') && (kpressed[0] <= '9'))) 	/* ...or numeric keys   */
    {
...
MakeMenu (MenuRoot * mr)
{
    ...
    for (cur = mr->first; cur != NULL; cur = cur->next)
    {
        ...
        else if ((enable_xft) && (Scr.StdFont.xftfont))
        {
            ...
            XftTextExtents8 (dpy, Scr.StdFont.xftfont, cur->item, cur->strlen, &extents);
            width = extents.xOff;
        }
#endif
        else
        ...
        else if ((enable_xft) && (Scr.StdFont.xftfont))
        {
            ...
            XftTextExtents8 (dpy, Scr.StdFont.xftfont, cur->item2, cur->strlen2, &extents);
            width = extents.xOff;
        }
#endif
        else
    ...
    for (y = 2, cur = mr->first; cur != NULL; cur = cur->next)
    {
        ...
        if (cur->func_type == F_TITLE)
        {
            ...
            if (cur->strlen2 == 0)
            {
                ...
                {
                    XRectangle rect1, rect2;
                    XmbTextExtents (fontset, cur->item, cur->strlen, &rect1,
                                    &rect2);
                    width = rect2.width;
                }
#ifdef HAVE_X11_XFT_XFT_H
