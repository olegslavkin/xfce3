...
StashEventTime (XEvent * ev)
{
  ...
  if ((NewTimestamp > CurrentTime)
      || ((CurrentTime - NewTimestamp) > 30000))
...
HandleFocusIn ()
{
    ...
    else if (Tmp_win != Scr.Hilite)
    {
        ...
        Broadcast (XFCE_M_FOCUS_CHANGE, 5, Tmp_win->w,
                   Tmp_win->frame, (unsigned long) Tmp_win,
                   GetDecor (Tmp_win, HiColors.fore),
                   GetDecor (Tmp_win, HiColors.back), 0, 0);
        if (Scr.ColormapFocus == COLORMAP_FOLLOWS_FOCUS)
        {
            if ((Scr.Hilite) && (!(Scr.Hilite->flags & ICONIFIED)))
            {
    ...
    for (key = Scr.AllBindings; key != NULL; key = key->NextBinding)
    {
        if ((key->Button_Key == Event.xkey.keycode) &&
                ((vmod (key->Modifier) == modifier)
                 || (key->Modifier == AnyModifier)) && (key->Context & Context)
                && (key->IsMouse == 0))
        {
...
HandlePropertyNotify ()
{
    ...
    if (Event.xproperty.atom == _XA_WIN_WORKSPACE_COUNT)
    {
        ...
        if (
            (XGetWindowProperty
             (dpy, Scr.Root, _XA_WIN_WORKSPACE_COUNT, 0L, 1L, False,
    ...
    case XA_WM_NAME:
        ...
        BroadcastName (XFCE_M_WINDOW_NAME, Tmp_win->w, Tmp_win->frame,
            ...
            if (!(Tmp_win->flags & SUPPRESSICON))
            {
                ...
                if (Tmp_win->flags & ICON_OURS)
                {
                    if (Tmp_win->icon_pixmap_w != None)
                    {
                        ...
                        XDeleteContext (dpy, Tmp_win->icon_pixmap_w,
                                        XfwmContext);
                    }
                }
                else
...
HandleExpose ()
{
    ...
    if (Tmp_win)
    {
        ...
        {
            SetBorder (Tmp_win, (Scr.Hilite == Tmp_win), False, True,
                       Event.xany.window);
        }
    }
#ifdef DEBUG
...
HandleMapNotify ()
{
    ...
    if (Tmp_win->flags & ICONIFIED)
        Broadcast (XFCE_M_DEICONIFY, 3, Tmp_win->w, Tmp_win->frame,
                   (unsigned long) Tmp_win, 0, 0, 0, 0);
    else
        Broadcast (XFCE_M_MAP, 3, Tmp_win->w, Tmp_win->frame,
                   (unsigned long) Tmp_win, 0, 0, 0, 0);
...
HandleUnmapNotify ()
{
    ...
    else if (XTranslateCoordinates (dpy, Event.xunmap.window, Scr.Root,
...
HandleButtonPress ()
{
    ...
    {
        if (
            ((MouseEntry->Button_Key == Event.xbutton.button)
...
HandleEnterNotify ()
{
    ...
    if (!(Scr.Options & ClickToFocus) &&
            !(e->focus) &&
            ((e->x_root != old_x_root) || (e->y_root != old_y_root) || (Scr.Options & ForceFocus)) &&
            (e->mode == NotifyNormal))
...
HandleConfigureRequest ()
{
    ...
    if (XFindContext (dpy, cre->window, XfwmContext, (caddr_t *) & Tmp_win) ==
            XCNOENT)
        Tmp_win = NULL;
    ...
    if (!Tmp_win || (Tmp_win->icon_w == cre->window))
    {
        ...
        if ((Tmp_win) && ((Tmp_win->icon_w == cre->window)))
        {
            ...
            if (!(Tmp_win->flags & ICON_UNMAPPED))
                Broadcast (XFCE_M_ICON_LOCATION, 7, Tmp_win->w, Tmp_win->frame,
    ...
    if (cre->value_mask & CWY)
        y =
            cre->y - Tmp_win->boundary_width - Tmp_win->title_height - Tmp_win->bw;
    if (cre->value_mask & CWWidth)
        width = cre->width + 2 * (Tmp_win->boundary_width + Tmp_win->bw);
    if (cre->value_mask & CWHeight)
        height =
            cre->height + Tmp_win->title_height + 2 * (Tmp_win->boundary_width + Tmp_win->bw);
    ...
    else
    {
        ...
        {			/* move desktop left */
            ...
            XWarpPointer (dpy, Scr.Root, Scr.Root,
                          ...
                          my_min(MyDisplayMaxX(warpx,warpy) - 1, warpx) - Scr.SnapSize,
        ...
        {			/* move desktop right */
            ...
            XWarpPointer (dpy, Scr.Root, Scr.Root,
                          0, 0, Scr.MyDisplayWidth, Scr.MyDisplayHeight,
                          my_min(MyDisplayX(warpx,warpy), warpx) + Scr.SnapSize,
                          my_min(MyDisplayMaxY(warpx,warpy) - 1, warpy));
            return;
        }
        ...
        {			/* move desktop upper */
            ...
            XWarpPointer (dpy, Scr.Root, Scr.Root,
                          ...
                          my_min(MyDisplayMaxY(warpx,warpy) - 1, warpy) - Scr.SnapSize);
            return;
        }
        if (Scr.EdgeScrollY >= (EDGEFACTOR * Scr.SnapSize))
        {			/* move desktop lower */
            ...
            XWarpPointer (dpy, Scr.Root, Scr.Root,
                          0, 0, Scr.MyDisplayWidth, Scr.MyDisplayHeight,
                          my_min(MyDisplayMaxX(warpx,warpy) - 1, warpx),
                          my_min(MyDisplayY(warpx,warpy), warpy) + Scr.SnapSize);
            return;
        }
    }
}
...
#ifdef __hpux
    retval =
        select (fd_width, (fd_set *) & in_fdset, (fd_set *) & out_fdset, 0, NULL);
#else
...
#ifdef HAVE_SESSION
    /* Weird : If alarm is set, ProcessICEMsgs () stucks */
