...
void
executeModule (XEvent * eventp, Window w, XfwmWindow * tmp_win,
...
HandleModuleInput (Window w, int channel)
{
    ...
    if (size > 255)
    {
        xfwm_msg (ERR, "HandleModuleInput",
                  "Module command is too big (%d)", (void *) ((long) size));
        size = 255;
    }
    ...
    if (strlen (text) > 0)
    {
        ...
        if (XFindContext (dpy, w, XfwmContext, (caddr_t *) & tmp_win) ==
                XCNOENT)
        {
            tmp_win = NULL;
            w = None;
...
SendConfig (int module, unsigned long event_type, XfwmWindow * t)
{
    ...
    body[HEADER_SIZE + 11] =
        (t->hints.flags & PBaseSize) ? t->hints.base_width : 0;
    body[HEADER_SIZE + 12] =
        (t->hints.flags & PBaseSize) ? t->hints.base_height : 0;
    body[HEADER_SIZE + 13] =
        (t->hints.flags & PResizeInc) ? t->hints.width_inc : 1;
    body[HEADER_SIZE + 14] =
        (t->hints.flags & PResizeInc) ? t->hints.height_inc : 1;
 ...
 */
void
SendStrToModule (XEvent * eventp, Window junk, XfwmWindow * tmp_win,
                 unsigned long context, char *action, int *Module)
{
    char *module, *str;
    int i;
...
void
send_list_func (XEvent * eventp, Window w, XfwmWindow * tmp_win,
                unsigned long context, char *action, int *Module)
{
    XfwmWindowList *t;
    ...
    if (*Module >= 0)
    {
        SendPacket (*Module, XFCE_M_NEW_DESK, 1, Scr.CurrentDesk, 0, 0, 0, 0, 0,
                    0);
        if (Scr.Hilite != NULL)
            SendPacket (*Module, XFCE_M_FOCUS_CHANGE, 5, Scr.Hilite->w,
                        Scr.Hilite->frame, (unsigned long) Scr.Hilite,
                        Scr.DefaultDecor.HiColors.fore,
                        Scr.DefaultDecor.HiColors.back, 0, 0);
        else
            SendPacket (*Module, XFCE_M_FOCUS_CHANGE, 5, 0, 0, 0,
                        Scr.DefaultDecor.HiColors.fore,
                        Scr.DefaultDecor.HiColors.back, 0, 0);
        ...
        for (t = LastXfwmWindowList(Scr.stacklist); t != NULL; t = t->prev)
        {
            ...
            SendName (*Module, XFCE_M_WINDOW_NAME, (t->win)->w, (t->win)->frame,
                      (unsigned long) (t->win), (t->win)->name);
            SendName (*Module, XFCE_M_ICON_NAME, (t->win)->w, (t->win)->frame,
                      (unsigned long) (t->win), (t->win)->icon_name);
            if ((t->win)->icon_bitmap_file != NULL &&
                    (t->win)->icon_bitmap_file != Scr.DefaultIcon)
                SendName (*Module, XFCE_M_ICON_FILE, (t->win)->w, (t->win)->frame,
                          (unsigned long) (t->win), (t->win)->icon_bitmap_file);
            SendName (*Module, XFCE_M_RES_CLASS, (t->win)->w, (t->win)->frame,
                      (unsigned long) (t->win), (t->win)->class.res_class);
            SendName (*Module, XFCE_M_RES_NAME, (t->win)->w, (t->win)->frame,
                      (unsigned long) (t->win), (t->win)->class.res_name);

            if ((t->win)->flags & ICONIFIED)
            {
                if (!((t->win)->flags & ICON_UNMAPPED))
                    SendPacket (*Module, XFCE_M_ICONIFY, 7, (t->win)->w, (t->win)->frame,
                                (unsigned long) (t->win),
                                (t->win)->icon_x_loc, (t->win)->icon_y_loc,
                                (t->win)->icon_w_width, (t->win)->icon_w_height + (t->win)->icon_p_height);
                else
                    SendPacket (*Module, XFCE_M_ICONIFY, 7, (t->win)->w, (t->win)->frame,
                                (unsigned long) (t->win), 0, 0, 0, 0);
            }
        }
        if (Scr.Hilite == NULL)
        {
            Broadcast (XFCE_M_FOCUS_CHANGE, 5, 0, 0, 0,
                       Scr.DefaultDecor.HiColors.fore,
                       Scr.DefaultDecor.HiColors.back, 0, 0);
        }
        else
        {
            Broadcast (XFCE_M_FOCUS_CHANGE, 5, Scr.Hilite->w,
                       Scr.Hilite->frame, (unsigned long) Scr.Hilite,
                       Scr.DefaultDecor.HiColors.fore,
                       Scr.DefaultDecor.HiColors.back, 0, 0);
        }
...
void
set_mask_function (XEvent * eventp, Window w, XfwmWindow * tmp_win,
                   unsigned long context, char *action, int *Module)
{
    int n, val1_unit;
    int val1;
