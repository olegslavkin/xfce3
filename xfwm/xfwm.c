...
main (int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        else if (mystrncasecmp (argv[i], "-f", 2) == 0)
        {
            ...
            {
                xfwm_msg (ERR, "main", "only %d -f and -cmd parms allowed!",
                          MAX_CFG_CMDS);
            }
        }
    ...
    {
        xfwm_msg (ERR, "main", "can't open display %s",
    ...
    if (Scr.Root == None)
    {
        xfwm_msg (ERR, "main", "Screen %d is not a valid screen",
                  (char *) Scr.screen);
        exit (1);
    }
    ...
    XSelectInput (dpy, Scr.Root,
                  ...
                  SubstructureNotifyMask | PointerMotionMask |	/* AutoDeskSwitch */
    ...
    attributes.override_redirect = True;
    Scr.GnomeProxyWin = XCreateWindow (dpy, Scr.Root, -10, -10, 10, 10, 0, 0,
                                    InputOnly, CopyFromParent,
                                    CWEventMask | CWOverrideRedirect,
                                    &attributes);
    XMapWindow (dpy, Scr.GnomeProxyWin);
    XChangeProperty (dpy, Scr.Root, _WIN_DESKTOP_BUTTON_PROXY, XA_CARDINAL, 32,
		     PropModeReplace, (unsigned char *) &Scr.GnomeProxyWin, 1);
    XChangeProperty (dpy, Scr.GnomeProxyWin, _WIN_DESKTOP_BUTTON_PROXY, XA_CARDINAL, 32,
		     PropModeReplace, (unsigned char *) &Scr.GnomeProxyWin, 1);
        ...
        for (i = 0; i < nchildren; i++)
        {
                              ...
                              (caddr_t *) & tmp) != XCNOENT)
            {
                ...
                if (XGetWindowProperty
                        (dpy, tmp->w, _XA_WM_STATE, 0L, 3L, False, _XA_WM_STATE,
                ...
                XChangeProperty (dpy, tmp->w, _XA_WM_DESKTOP, _XA_WM_DESKTOP,
                                 32, PropModeReplace, (unsigned char *) data,
                                 1);

                data[0] =
                    (unsigned long) (tmp->flags & (STICKY | ICON_MOVED | SHADED));
                XChangeProperty (dpy, tmp->w, _XA_XFWM_FLAGS, _XA_XFWM_FLAGS,
                                 32, PropModeReplace, (unsigned char *) data,
                                 1);
                data[0] = (unsigned long) tmp->icon_x_loc;
                XChangeProperty (dpy, tmp->w, _XA_XFWM_ICONPOS_X,
                                 _XA_XFWM_ICONPOS_X, 32, PropModeReplace,
                                 (unsigned char *) data, 1);
                data[0] = (unsigned long) tmp->icon_y_loc;
                XChangeProperty (dpy, tmp->w, _XA_XFWM_ICONPOS_Y,
                                 _XA_XFWM_ICONPOS_Y, 32, PropModeReplace,
                                 (unsigned char *) data, 1);
...
SetRCDefaults ()
{
    ...
    char *defaults[] = {
        ...
        "TitleStyle Active Solid #5F7EAB",
        "TitleStyle Inactive Solid #C2C2C2",
        "MenuColor black #DCDCDC white #5F7EAB",
...
CreateCursors (void)
{
    ...
    Scr.XfwmCursors[BOTTOM_LEFT] =
        XCreateFontCursor (dpy, XC_bottom_left_corner);
    Scr.XfwmCursors[BOTTOM_RIGHT] =
...
LoadDefaultLeftButton (ButtonFace * bf, int i)
{
    ...
    case 0:
        ...
        bf->vector.num = 5;
        bf->bitmap =
            XCreateBitmapFromData (dpy, Scr.Root, menu_bits, menu_width,
                                   menu_height);
    ...
    case 1:
        ...
        bf->vector.num = 9;
        bf->bitmap =
            XCreateBitmapFromData (dpy, Scr.Root, stick_bits, stick_width,
                                   stick_height);
        bf->bitmap_pressed =
            XCreateBitmapFromData (dpy, Scr.Root, stick_pressed_bits,
                                   stick_pressed_width, stick_pressed_height);
        break;
    case 2:
        bf->vector.x[0] = 20;
        bf->vector.y[0] = 36;
        ...
        bf->vector.num = 13;
        bf->bitmap =
            XCreateBitmapFromData (dpy, Scr.Root, shade_bits, shade_width,
                                   shade_height);
        bf->bitmap_pressed =
            XCreateBitmapFromData (dpy, Scr.Root, shade_pressed_bits,
                                   shade_pressed_width, shade_pressed_height);
        break;
    default:
        break;
    }
...
LoadDefaultRightButton (ButtonFace * bf, int i)
{
    ...
    case 0:
        ...
        bf->vector.num = 13;
        bf->bitmap =
            XCreateBitmapFromData (dpy, Scr.Root, close_bits, close_width,
                                   close_height);
        bf->bitmap_pressed = None;
        break;
    case 1:
        ...
        bf->vector.num = 5;
        bf->bitmap =
            XCreateBitmapFromData (dpy, Scr.Root, maximize_bits, maximize_width,
                                   maximize_height);
        bf->bitmap_pressed =
            XCreateBitmapFromData (dpy, Scr.Root, maximize_pressed_bits,
                                   maximize_pressed_width,
                                   maximize_pressed_height);
        break;
    case 2:
        bf->vector.x[0] = 42;
        bf->vector.y[0] = 42;
        ...
        bf->vector.num = 5;
        bf->bitmap =
            XCreateBitmapFromData (dpy, Scr.Root, iconify_bits, iconify_width,
                                   iconify_height);
        bf->bitmap_pressed = None;
        break;
    default:
...
InitVariables (void)
{
    ...
    Scr.ndesks = 1;
    {
        ...
        if (
            (XGetWindowProperty
             (dpy, Scr.Root, _XA_WIN_WORKSPACE_COUNT, 0L, 1L, False, XA_CARDINAL,
...
Done (int restart, char *command)
{
    ...
    if (restart == XFWM_RESTART)
    {
        ...
        xfwm_msg (ERR, "Done",
                  "Call of '%s' failed!!!! (restarting '%s' instead)", command,
                  g_argv[0]);
...
usage (void)
{
    ...
    fprintf (stderr,
             "  %s [-d dpy] [-f config_cmd] [-s] [-blackout] [-version] [-h]\n"
    ...
    for (t = Scr.XfwmRoot.next; t != NULL; t = t->next)
    {
        ...
        data[0] =
            (unsigned long) (t->flags &
                             (STICKY | ICONIFIED | ICON_MOVED | SHADED |
                              WM_NAME_CHANGED | RECAPTURE));
