...
RestackWindow (XfwmWindow * t, Bool top)
{
    ...
    for (j = MAX_LAYERS; j >= 0; j--)
    {
        ...
        {
            if ((t2->win != t) &&
                    !(((t2->win)->flags & TRANSIENT) && ((t2->win)->transientfor == t->w)) &&
                    ((t2->win)->layer == j))
            {
	        newstack = PushStackList (newstack, wins, t2->win, &i);
            }
...
Bool GetWindowLayer (XfwmWindow * tmp)
{
    Atom atype;
    int aformat;
    unsigned long nitems, bytes_remain;
    unsigned char *prop;
    if ((XGetWindowProperty (dpy, tmp->w, _XA_WIN_LAYER, 0L, 1L, False,
...
Window
SearchWindowList(WindowList* start, Window t)
{
    WindowList *i;
