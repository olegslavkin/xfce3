...
master_dialog (char *ip)
{
  ...
  gtk_signal_connect (GTK_OBJECT (master), "key-press-event",
		      GTK_SIGNAL_FUNC (entry_keypress_master), NULL);
  ...
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      GTK_SIGNAL_FUNC (ok_dialog_master), (gpointer) dialog);
  button = gtk_button_new_with_label ("Cancel");
  gtk_box_pack_start (GTK_BOX (GTK_DIALOG (dialog)->action_area),
		      button, EXPAND, NOFILL, 0);
  gtk_widget_show (button);
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      GTK_SIGNAL_FUNC (destroy_dialog_master), (gpointer) dialog);
  gtk_widget_show (dialog);
  gtk_widget_grab_focus (master);
  return dialog;
}

void ask4master(void){
	FILE *archie;
	char *home,line[MAXSTRLEN];
	home=getenv("HOME");
	snprintf(line, MAXSTRLEN, "%s/.xfce/lmhosts.xfsamba", home);
	...
	if (archie){
       ...
       gtk_window_set_transient_for (GTK_WINDOW (master_dialog (thisN->serverIP)),
				    GTK_WINDOW (smb_nav));
       gtk_main();
       return;	  
}  
...
again_master:
  ...
  if (thisN->next) thisN = thisN->next;
  else /* alert no master browser resolved on network */
    {	
       ask4master();
       
      if (thisN->netbios) goto again_master;
      {    
       print_status (_
...
NMBmastersResolve (nmb_list * currentN)
{
  ...
  fork_obj = 
	  Tubo (NMBmastersResolveFork, NMBmastersResolveOver, FALSE,
