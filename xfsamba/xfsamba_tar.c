...
SMBtarFork (void)
{
  ...
  argument[i++]="smbclient";
    ...
    argument[i++]="-T";
    if (tar_options&0xf00){argument[i++]="x";}
    else {
      switch (tar_options&0xff){
       case 0x11: argument[i++]="cga";break;
       case 0x01: argument[i++]="cg";break;
       case 0x10: argument[i++]="ca";break;
       default: argument[i++]="c";break;
      }
    }
    argument[i++]=tarO;
#ifdef DBG_XFSAMBA
    {
     int j;
     for (j=0;j<i;j++) fprintf(stderr,"%s ",argument[j]); fprintf(stderr,"\n");
     fflush(NULL);
    }
#endif
     argument[i++]=(char *) 0;
...
SMBtarForkOver (void)
{
  ...
  switch (SMBResult)
    ...
    default:
      ...
      else {print_status (_("Tar command complete."));}
       break;
    }
}
...
proceed_tar (GtkWidget * widget, gpointer data)
{
  ...
  if (ok)
    {  
      ...
      tarO=(char *)malloc(strlen(thisN->netbios)
		      +strlen(selected.share)
		      +strlen(selected.dirname)+3+strlen(".tar"));

      if (tar_options&0xf00) {strcpy(tarO,"*.tar");}
      else {
...
error_3361:
  ...
  sprintf (tarO, "%s",fileS);
      ...
      fork_obj = Tubo (SMBtarFork, SMBtarForkOver, TRUE,
		       SMBtarStdout, parse_stderr);
    }
  else
    {
      cursor_reset (GTK_WIDGET (smb_nav));
...
static void toggle_tar_options(GtkWidget * widget, gpointer data){
	int choice;
	choice=(int )((long)data);
	switch (choice) {
		case 1: tar_options |= 0x01;
			break;
		case 0: tar_options &= 0xff0;
			break;
		case 3: tar_options |= 0x10;
			break;
		case 2: tar_options &= 0xf0f;
			break;
		case 5: tar_options |= 0x100;
			break;
		case 4: tar_options &= 0x0ff;
			break;
	}
}
...
SMBtarDialog (void)
{
  ...
  char *pathname;
  tar_options=0x0;
  pathname=(char *) malloc(2+strlen(thisN->server)+
		  	   1+strlen(selected.share)+
			   strlen(selected.dirname)+
			   1+((!selected.file)?0:strlen(selected.filename))
			   +1);
...
notebook = gtk_notebook_new ();{
 ...
 scrolled = gtk_scrolled_window_new (NULL, NULL);{
  ...
  vbox = gtk_vbox_new (FALSE, 0);{
   ...
   gtk_container_set_focus_vadjustment 
	  (GTK_CONTAINER (vbox),gtk_scrolled_window_get_vadjustment
   ...
   hbox = gtk_hbox_new (FALSE, 0);{
    ...
    for (i=4;i<6;i++) {
     ...
     gtk_signal_connect (GTK_OBJECT (button), "clicked",
		     GTK_SIGNAL_FUNC (toggle_tar_options),(gpointer)((long) i)); 
 ...
 gtk_notebook_set_tab_label ((GtkNotebook *)notebook,scrolled,label);
  ...
  gtk_widget_show (button);
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      GTK_SIGNAL_FUNC (proceed_tar), (gpointer)((long) 1));
  ...
  gtk_widget_show (button);
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      GTK_SIGNAL_FUNC (proceed_tar), (gpointer)((long) 0));
...
void SMBtar (void){
  ...
  stopcleanup=FALSE;
	  gtk_window_set_transient_for (GTK_WINDOW
				  (SMBtarDialog ()), GTK_WINDOW (smb_nav));
}
