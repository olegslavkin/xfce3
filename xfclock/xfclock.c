...
gint update_xfclock_date_timer (GtkWidget * widget)
{
  time_t ticks;
  struct tm *tm;
  static gint mday = -1;
  ...
  ticks = time (0);
  tm = localtime (&ticks);
  gtk_calendar_get_date (GTK_CALENDAR (widget), &cal_year, &cal_mon,
			 &cal_day);
  if ((mday != tm->tm_mday) || (mon != tm->tm_mon) || (year != tm->tm_year)
      || (cal_day != tm->tm_mday) || (cal_mon != tm->tm_mon)
      || (cal_year != tm->tm_year))
    {
      mday = tm->tm_mday;
      mon = tm->tm_mon;
...
main (int argc, char *argv[])
{
  ...
  if ((current_config.fore_red >= 0) &&
      (current_config.fore_green >= 0) && 
      (current_config.fore_blue >= 0))
    {
  ...
  if ((current_config.back_red >= 0) &&
      (current_config.back_green >= 0) && 
      (current_config.back_blue >= 0))
    {
  ...
  my_gtk_clock_show_military (MY_GTK_CLOCK (clock_widget),
			      current_config.military);
  my_gtk_clock_show_secs (MY_GTK_CLOCK (clock_widget),
			  current_config.seconds);
  ...
  gtk_signal_connect (GTK_OBJECT (eventbox), "button_press_event",
		      GTK_SIGNAL_FUNC (show_popup_cb),
		      (gpointer) clock_widget);

  gtk_signal_connect (GTK_OBJECT (window), "delete_event",
		      GTK_SIGNAL_FUNC (delete_event), NULL);
  gtk_signal_connect (GTK_OBJECT (calendar), "month_changed", 
                           GTK_SIGNAL_FUNC (monthchanged),
                           NULL);
  gtk_signal_connect (GTK_OBJECT(calendar), "day_selected_double_click",
		      GTK_SIGNAL_FUNC (edit_appointments), NULL);
