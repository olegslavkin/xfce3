...
confirm_quit (XFMENU *xfmenu)
{
	...
	quit_dialog = my_show_dialog (message, nbuttons, buttons, default_button, 
				      signal_handlers, xfmenu);
	gtk_signal_connect (GTK_OBJECT (quit_dialog), "delete_event", 
	                    GTK_SIGNAL_FUNC (confirm_quit_no), NULL);
...
gboolean
confirm_delete (GtkCTreeNode *node)
{
	...
	delete_dialog = my_show_dialog (message, nbuttons, buttons, default_button, 
				        signal_handlers, node);
	gtk_signal_connect (GTK_OBJECT (delete_dialog), "delete_event", 
	                    GTK_SIGNAL_FUNC (confirm_delete_no), NULL);
...
gboolean
on_xfumed_window_delete_event          (GtkWidget       *widget,
                                        GdkEvent        *event,
                                        gpointer         user_data)
{
...
gboolean
on_xfumed_window_destroy_event         (GtkWidget       *widget,
                                        GdkEvent        *event,
                                        gpointer         user_data)
{
	gtk_main_quit ();
...
void
on_ctree_tree_collapse                 (GtkCTree        *ctree,
                                        GList           *node,
                                        gpointer         user_data)
{
}
...
void
on_ctree_tree_expand                   (GtkCTree        *ctree,
                                        GList           *node,
                                        gpointer         user_data)
{
...
void
on_ctree_drag_begin                    (GtkWidget       *widget,
                                        GdkDragContext  *drag_context,
                                        gpointer         user_data)
{
	XFMENU *xfmenu = (XFMENU*) user_data;

	gtk_signal_handler_block_by_func (GTK_OBJECT (xfmenu->ctree), 
	                                  (GtkSignalFunc) on_ctree_button_release_event, 
	                                  xfmenu);
}


void
on_ctree_drag_data_delete              (GtkWidget       *widget,
                                        GdkDragContext  *drag_context,
                                        gpointer         user_data)
{
}
...
void
on_ctree_drag_data_get                 (GtkWidget       *widget,
                                        GdkDragContext  *drag_context,
...
void
on_ctree_drag_data_received            (GtkWidget       *widget,
                                        GdkDragContext  *drag_context,
                                        gint             x,
                                        gint             y,
                                        GtkSelectionData *data,
                                        ...
                                        gpointer         user_data)
{
	...
	if (!GTK_IS_CTREE (widget) || 
	    widget != gtk_drag_get_source_widget (drag_context))
		return;
	
	row = -1;
	y -= GTK_CLIST (xfmenu->ctree)->column_title_area.height;
	
	if (gtk_clist_get_selection_info (GTK_CLIST (xfmenu->ctree), x, y, &row, NULL)
	    && info == TARGET_ID
	    && row >= 0)
	{
		...
		if (node != sibling) {
			...
			xfmenu->first_node = gtk_ctree_node_nth (GTK_CTREE(xfmenu->ctree),
			                                         0);
			xfmenu->saved = FALSE;
		}
	}
...
void
on_ctree_drag_end                      (GtkWidget       *widget,
                                        GdkDragContext  *drag_context,
                                        gpointer         user_data)
{
	XFMENU *xfmenu = (XFMENU*) user_data;

	gtk_signal_handler_unblock_by_func (GTK_OBJECT (xfmenu->ctree), 
	                                    (GtkSignalFunc) on_ctree_button_release_event, 
	                                    xfmenu);
}
...
gboolean
on_ctree_button_press_event            (GtkWidget       *widget,
                                        ...
                                        gpointer         user_data)
{
	...
	if (event->button <= 3 &&
	    gtk_clist_get_selection_info (GTK_CLIST (xfmenu->ctree), x, y, &row, NULL))
	{
...
gboolean
on_ctree_button_release_event            (GtkWidget       *widget,
                                        GdkEventButton  *event,
                                        gpointer         user_data)
{
	XFMENU *xfmenu = (XFMENU*) user_data;
                                        ...
                                        gpointer         user_data)
{
	...
	if (prevnode) {
		...
		if (gtk_ctree_node_is_visible (GTK_CTREE (xfmenu->ctree), node) != 
		    GTK_VISIBILITY_FULL)
			gtk_ctree_node_moveto (GTK_CTREE (xfmenu->ctree), node, 
		                               0.0, 0,0);
	}
}
                                        ...
                                        gpointer         user_data)
{
	...
	while (nextnode && 
	       gtk_ctree_is_ancestor (GTK_CTREE (xfmenu->ctree), node, nextnode))		
	{
	...
	while (nextnext && 
	       gtk_ctree_is_ancestor (GTK_CTREE (xfmenu->ctree), nextnode, nextnext))		
	{
                                        ...
                                        gpointer         user_data)
{
	...
	if (GTK_CTREE_ROW (xfmenu->selected_node)->is_leaf) {
		gtk_ctree_node_get_pixtext (GTK_CTREE (xfmenu->ctree), node, 0, 
...
gboolean
on_ctree_key_press_event         (GtkWidget       *widget,
                                  GdkEventKey     *event,
                                  gpointer         user_data)
{
	XFMENU *xfmenu = (XFMENU*) user_data;
	...
	case GDK_Up:
		if (GTK_CTREE_NODE_PREV(node))
			gtk_ctree_select(GTK_CTREE (xfmenu->ctree), 
...
void
on_fileselect_button_clicked           (GtkButton       *button,
                                        gpointer         user_data)
{
	XFMENU *xfmenu = (XFMENU*) user_data;
	XFDLG *dlg = xfmenu->dlg;
...
void
on_dlg_ok_button_clicked               (GtkButton       *button,
                                        gpointer         user_data)
{
	XFMENU *xfmenu = (XFMENU*) user_data;
	XFDLG *dlg = xfmenu->dlg;
...
void
on_dlg_cancel_button_clicked           (GtkButton       *button,
                                        gpointer         user_data)
{
	XFMENU *xfmenu = (XFMENU*) user_data;
	XFDLG *dlg = xfmenu->dlg;
...
gboolean
on_edit_dialog_key_press_event         (GtkWidget       *widget,
                                        GdkEventKey     *event,
                                        gpointer         user_data)
{
	switch (event->keyval)
