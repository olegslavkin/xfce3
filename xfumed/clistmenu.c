...
clist_row_moveup(GtkCList *clist, gint row)
{
	...
	if (item!=NULL)
	{
		...
		menu_clist->entries=g_list_insert(menu_clist->entries, 
											(gpointer) item, row-1);
	}
...
clist_to_parent(GtkCList *clist, RootMenu *current_root)
{
	if ( current_root->parent==NULL ||
		 (menu_clist=menu_find_by_name(current_root->parent))==NULL )
		return;
...
entries_set_text(gchar *type, gchar *caption, gchar *command)
{
	...
	{
		gtk_entry_set_text(GTK_ENTRY(combo_entry_type),
                                                TYPE_SUBMENU_TEXT);

		if (caption==NULL) text=g_strdup("");
		else text=g_strdup(caption);
	...
	else if (g_strcasecmp(type,TYPE_ITEM_TEXT)==0)
	{
		...
		if (command==NULL) text=g_strdup("");
		else text=g_strdup(command);
