...
gchar *menu_get_caption(RootMenu *menu)
{
  ...
  if ( menu->parent==NULL ||
       (parent=menu_find_by_name(menu->parent))==NULL )
...
void read_menu(void)
{
	...
	configName = g_strconcat(g_getenv("HOME"),
				"/.xfce/xfwm.user_menu",NULL);
	...
	*/
	...
	while (get_line(configFile,line,MAXSTRLEN)!=0)
	{
		...
		if (g_strcasecmp(word,"AddToMenu")==0)
		{
			...
			if ((word=get_next_word(remainder))==NULL ||
				strlen(word)==0)
				continue;
...
void write_menu(GList *menulist)
{
	...
	else if ( (configFile=fopen(configName,"r"))!=NULL &&
		 (backupFile=fopen(backupName,"w"))!=NULL )
	{
		gint chr;
