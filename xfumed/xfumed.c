...
GtkWidget *read_menu (XFMENU *xfmenu)
{
	...
	while (fgets (line,MAXSTRLEN -1, fp) != NULL) {
		...
		if (g_strcasecmp ("AddToMenu",word) == 0) {
			...
			if (!(have_menu = find_node_by_name (xfmenu, menuname, 
				                             &current_node)))
...
gboolean find_node_by_name (XFMENU *xfmenu, char *name, GtkCTreeNode **return_node)
{
	GList *list;
...
GtkCTreeNode *update_node (XFMENU *xfmenu, char **text)
{
	...
	if (is_leaf) {
		...
		gtk_ctree_set_node_info (GTK_CTREE (xfmenu->ctree), node, 
	        	                 text[0], SPACING, 
	                 	         pm, bm, NULL, NULL, is_leaf, 0);
	} else {
		gtk_ctree_set_node_info (GTK_CTREE (xfmenu->ctree), node, 
	        	                 text[0], SPACING, 
	                 	         pix_closed, bm_closed,
	                        	 pix_open, bm_open,
	                         	 is_leaf, 0);
	}
...
GtkCTreeNode *insert_node(XFMENU *xfmenu, GtkCTreeNode *parent, GtkCTreeNode *sibling, 
                       char **text, gboolean is_leaf)
{
	GtkCTreeNode *node = NULL;
	
	if (is_leaf) {
		if (strcmp(NOP_CAPTION, text[0]) == 0)
			node = gtk_ctree_insert_node (GTK_CTREE (xfmenu->ctree), 
		                                      parent, sibling, 
		                                      text, SPACING,
		                                      NULL, NULL, NULL, NULL,
		                                      is_leaf, 0);
			...
			node = gtk_ctree_insert_node (GTK_CTREE (xfmenu->ctree), 
		                                      parent, sibling, 
		                                      text, SPACING,
		                                      pix_exe, bm_exe, NULL, NULL,
		                                      is_leaf, 0);
	} else {
		if (find_node_by_name(xfmenu, text[2], &node)) {
			if (strcmp(TEMPNODETEXT, text[0]) == 0) {
		
				if (gtk_ctree_node_is_visible(GTK_CTREE (xfmenu->ctree), 
					                      node)!=GTK_VISIBILITY_FULL)
					gtk_ctree_node_moveto (GTK_CTREE (xfmenu->ctree),
				                               node, 0.5, 0,0);
			...
			xfmenu->menulist = g_list_append(xfmenu->menulist, 
			                                 (gpointer) node);
		}
	}
...
void remove_from_list (GtkCTree *ctree, GtkCTreeNode *node, gpointer data)
{
...
{
	gtk_ctree_post_recursive (GTK_CTREE (xfmenu->ctree), node, 
	                          (GtkCTreeFunc) remove_from_list, 
	                          (gpointer) xfmenu);
...
void write_menu (XFMENU *xfmenu)
{
	...
	gtk_ctree_post_recursive(GTK_CTREE (xfmenu->ctree), NULL, 
	                         (GtkCTreeFunc) write_node, (gpointer) fp);
