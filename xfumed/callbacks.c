...
gboolean 
on_xfumed_delete_event(GtkWidget *widget, GdkEvent *event,
                                  gpointer user_data)
{
  quit();
  return FALSE;
...
gboolean
on_xfumed_destroy_event(GtkWidget *widget, GdkEvent *event,
                          gpointer user_data)
{
  gtk_main_quit();
  return TRUE;
...
void
on_clist_menuitems_select_row (GtkCList *clist, gint row, gint column,
...
void
on_clist_menuitems_unselect_row (GtkCList *clist, gint row, gint column,
                                   GdkEvent *event, gpointer user_data)
{
...
on_combo_entry1_changed (GtkEditable *editable, gpointer user_data)
{
  ...
  if (g_strcasecmp(type,TYPE_NOP_TEXT)==0)
  {
    ...
    if (fileselect_blocked!=1)
    {
      ...
      gtk_signal_handler_block_by_func(GTK_OBJECT(button_fileselect), 
      			GTK_SIGNAL_FUNC(on_button_fileselect_clicked), NULL);
    }
  ...
  if (fileselect_blocked==1)
  {
    ...
    gtk_signal_handler_unblock_by_func(GTK_OBJECT(button_fileselect), 
    			GTK_SIGNAL_FUNC(on_button_fileselect_clicked), NULL);
  }
...
gboolean
on_quit_dialog_delete(GtkWidget *widget, GdkEvent *event, gpointer data)
{
  return TRUE;
}
