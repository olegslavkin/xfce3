...
#else
    input_id = gdk_input_add (fd_internal_pipe[1],
			      GDK_INPUT_READ | GDK_INPUT_EXCEPTION,
			      (GdkInputFunction) process_xfwm_messages,
			      (gpointer) ((long)fd_internal_pipe[1]));
#endif
}
  ...
  if ((p) && (current_config.wm == XFWM))
    {
      ...
      howdark = brightness_pal (p, 7);
      color_to_hex (t, p, 2);
      sprintf (u, MENUCOLOR_CMD, (howdark < fadeblack) ? "white" : "black", s,
  ...
  if (current_config.wm == XFWM)
    {
      ...
      sendinfo (fd_internal_pipe,
		(current_config.autoraise ? AUTORAISEON_CMD :
		 AUTORAISEOFF_CMD), 0);
      sendinfo (fd_internal_pipe,
		(current_config.mapfocus ? MAPFOCUSON_CMD :
		 MAPFOCUSOFF_CMD), 0);
    }
}
  ...
  if (current_config.wm == XFWM)
    {
      ...
      for (i = 0; i < ndesks; i++)
        {
          snprintf (command, 255, ADDTOMENUDESK_CMD, DESKMENU, 
