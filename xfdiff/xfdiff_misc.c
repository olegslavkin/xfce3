...
xf_button (GtkWidget * widget, gpointer data)
{
  int caso;
  caso=(int)((long)data); 
  if (caso) xf_result=TRUE; 
  gtk_main_quit();
  return;
}

static GtkWidget *xf_selection(char *title,int caso,char *aux){
  ...
  switch (caso){
   case FONT_SELECTION:
	...
	gtk_font_selection_dialog_set_preview_text
		(GTK_FONT_SELECTION_DIALOG(dialogo),
		 "Xfce is at http://www.xfce.org/");
	if (!gtk_font_selection_dialog_set_font_name
	    (GTK_FONT_SELECTION_DIALOG(dialogo),aux)){
   ...
   case COLOR_SELECTION:
	...
	ko_object=GTK_OBJECT(GTK_COLOR_SELECTION_DIALOG (dialogo)->cancel_button);
  	gtk_color_selection_set_color (GTK_COLOR_SELECTION
	    (GTK_COLOR_SELECTION_DIALOG (dialogo)->colorsel),(gdouble *)aux);	
	break;
  ...
  gtk_signal_connect (ok_object,"clicked",
  	GTK_SIGNAL_FUNC (xf_button),(gpointer) ((long)1));
  gtk_signal_connect (ko_object,"clicked",
	GTK_SIGNAL_FUNC (xf_button), (gpointer) ((long)0));
...
static GtkWidget *xf_dialog(char *message,int caso){
  static GtkWidget *dialogo;
  GtkWidget *hbox, *label;

  switch (caso){
	  case 1: dialogo = gtk_dialog_new_with_buttons 
		  	(_("Confirm"),GTK_WINDOW(diff),GTK_DIALOG_MODAL,
			 _("OK"),GTK_RESPONSE_ACCEPT,_("Abort"),GTK_RESPONSE_REJECT,
			 NULL);
 		  break;				  
	  case 2: dialogo = gtk_dialog_new_with_buttons 
		  	(_("Question"),GTK_WINDOW(diff),GTK_DIALOG_MODAL,
			 _("Yes"),GTK_RESPONSE_ACCEPT,_("No"),GTK_RESPONSE_REJECT,
			 NULL);
		  break;	
	  default:dialogo = gtk_dialog_new_with_buttons 
		  	(_("Alert"),GTK_WINDOW(diff),GTK_DIALOG_MODAL,
  ...
  gtk_box_pack_start (GTK_BOX (GTK_DIALOG (dialogo)->vbox), hbox,
		      TRUE, TRUE, 0);
...
static gboolean xf_hey(char *message,int caso){
  GtkWidget *widget;
  gint result;
...
char *xf_selector(char *title,int caso,char *aux){
  ...
  if (xf_result) {
   switch (caso){
    case FONT_SELECTION:
	name=(char *)malloc(strlen(gtk_font_selection_dialog_get_font_name
          (GTK_FONT_SELECTION_DIALOG (widget)))+1);
	strcpy (name,gtk_font_selection_dialog_get_font_name 
	  (GTK_FONT_SELECTION_DIALOG (widget)));
printf("out=%s\n",name);
	break;
    case COLOR_SELECTION:
     	gtk_color_selection_get_color (GTK_COLOR_SELECTION
	  (GTK_COLOR_SELECTION_DIALOG (widget)->colorsel), &selcolor);
	gtk_widget_destroy (widget); 
	return (char *)(&selcolor);	
  	break;
    default: /* FILE_SELECTION */
	name=(char *)malloc(strlen(gtk_file_selection_get_filename 
	  (GTK_FILE_SELECTION (widget)))+1);
	strcpy (name,gtk_file_selection_get_filename 
...
char *drawA_width(void){
	if ((rightC<1000)&&(leftC<1000)) return "88800888";
	if ((rightC<10000)&&(leftC<10000)) return "8888008888";
	if ((rightC<100000)&&(leftC<100000)) return "888880088888";
...
static int go_figure_strip(void){
	patched_file* thisF;
...
chance2:
	...
	if (!hits) xfdiff_abort(E_MALLOC);
	filename = (char *)malloc(Mlength + strlen(fileD) + 2);
	if (!filename) xfdiff_abort(E_MALLOC);
	for (i=0;i<=Mstrip;i++) {
		*(hits+i)=0;
		thisF=headF;
		while (thisF) {
	...
	for (i=0;i<=Mstrip;i++) {
		...
		if ( *(hits+i) <= Mlength) continue;
		Mlength = *(hits+i);
		j = i;			
	}	

	if (Mlength == 0) free(filename);
	free(hits);
	if ((ch)&&(Mlength == 0)) { /* second chance */
		ch=0;
		goto chance2;
	}
	return j;
...
void cleanA(void){
	...
	gdk_draw_rectangle (drawP,drawA->style->bg_gc[GTK_WIDGET_STATE (drawA)],
	                   TRUE,0, 0,
	                   drawA->allocation.width,3*drawA->allocation.height);
	gtk_widget_draw (drawA, &update_rect);
	gdk_draw_pixmap(drawA->window,
	               drawA->style->bg_gc[GTK_WIDGET_STATE (drawA)],
	               drawP,0,0,0,0, 
	               drawA->allocation.width,drawA->allocation.height);
}
...
#else	
	if (text_left) gtk_text_backward_delete (GTK_TEXT(text_left), 
		          gtk_text_get_length (GTK_TEXT(text_left) ) );	
	if (text_right) gtk_text_backward_delete (GTK_TEXT(text_right), 
...
char *prompt_path(char *message,char *file){
...
#ifdef __GTK_2_0	
	/*  */	
	if (!xf_confirm(texto)) return NULL;
#else	
	...
	free(texto); return file;
}

char *prompt_outfile(char *message,char *file){
#ifdef __GTK_2_0	
	if (!xf_confirm(message)) return NULL;
#else	
...
void set_the_style(GtkWidget *widget){
  ...
  if (xfce_style) style = gtk_style_copy(xfce_style);
  else style=gtk_style_new(); 
		...
		while ((!feof(defaults)) && (fgets(line,255,defaults))) 
		  {
		    ...
		    if (strstr(word,"synchronize"))  synchronize  = atoi(value);
		    if (strstr(word,"font")) {
			    if (strstr(value,"\n")) value = strtok(value,"\n");
...
/*	tagT=gtk_text_tag_table_new();
	...
	gtk_text_buffer_create_tag(text_right,"bluetext",
			"background-gdk",filebg,
			"foreground-gdk",filefg,NULL);
	gtk_text_buffer_create_tag(text_left,"bluetext",
			"background-gdk",filebg,
			"foreground-gdk",filefg,NULL);
#endif
...
// font color!
...
#else	
		if (what_dir) gtk_text_backward_delete (GTK_TEXT(what_dir), 
			          gtk_text_get_length (GTK_TEXT(what_dir) ) );
		if (!fileP) {
			if (what_dir) gtk_text_insert (GTK_TEXT (what_dir),
				NULL,&filefg,&filebg,no_patch,strlen (no_patch));
		}
		else {
			if (what_dir) gtk_text_insert (GTK_TEXT (what_dir),NULL,NULL,NULL,
				_("Patch: "),strlen (_("Patch: ")));
			if (what_dir) gtk_text_insert (GTK_TEXT (what_dir),NULL,&filefg,&filebg,
				fileP,strlen (fileP));
		}
		if (what_dir) gtk_text_insert (GTK_TEXT (what_dir),NULL,NULL,NULL,
		...
		if (!fileP) texto=no_patch; 
			...
			else texto=(_("Press OK to see patch diffs"));
		}		
	} else { /* !patching */
		if (!fileR) texto = no_right_path;  else texto = fileR;  
	}
	...
	if (thisF) {
		...
		if (titleD) gtk_combo_set_popdown_strings (GTK_COMBO (titleD),items);
	} else {
		if (!patching) {
			if (fileL) texto=fileL;
			else texto=no_left_path;
		}
		items = g_list_append (items, texto);	
		if (titleP)gtk_combo_set_popdown_strings (GTK_COMBO (titleD),items);
	}
}
