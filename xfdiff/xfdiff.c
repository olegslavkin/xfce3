...
static char *cannot_read_patch_file,
	*identical_files,*patch_file_error,
	*empty_file;

#ifdef __GTK_2_0	
void utf8_insert_blue(GtkTextBuffer *buffer,char *line,int linecount){
...
void utf8_insert(GtkTextBuffer *buffer,char *line,int linecount){
  ...
  } else gtk_text_buffer_insert_at_cursor (buffer,line,linecount);
}	
...
#endif
static void assign_tmp_files(void){
	char pidT[32];
	char *tmp_dir=NULL;
...
int pushemL(char *file){
	...
	if (!currentF->newfile) xfdiff_abort(E_MALLOC);
...
static int checkfile(char *file){
	...
	if ((check=fopen(file,"r"))==NULL) return FALSE;
	fclose(check); return TRUE;
}

#ifdef __GTK_2_0	
static void writebinary(GtkTextBuffer *text_location){
	// missing color attribute
  utf8_insert(text_location,_("****Binary data****\n"),-1);
}
#else	
static void writebinary(GtkWidget *text_location){
	gtk_text_insert (GTK_TEXT (text_location),style->font,&filefg,
			&filebg,_("****Binary data****\n"),
			strlen(_("****Binary data****\n")));
}
#endif

#ifdef __GTK_2_0	
static void writeout(GtkTextBuffer *text_location,char *file){
	int fileH;
	fileH=open(file,O_RDONLY);
	if (fileH==-1){ 
  utf8_insert(text_location,empty_file,-1);
#else	
static void writeout(GtkWidget *text_location,char *file){
	int fileH;
	fileH=open(file,O_RDONLY);
	if (fileH==-1){ 	
		gtk_text_insert (GTK_TEXT (text_location),
			style->font,&filefg,
			&filebg,empty_file,strlen(empty_file));
#endif	
	}
	else {
		char *blok;
...
#else	
			gtk_text_insert (GTK_TEXT (text_location),
				style->font,NULL,NULL,blok,blok_s);
#endif
	...
	if (!patchO) { /* not creating a patchfile */
		...
		if ( checkdir(fileL) || checkdir(fileR)){
			...
			if (checkdir(fileL)) {
				...
				if (!tmp_c) xfdiff_abort(E_MALLOC);			
				sprintf(tmp_c,"%s/%s",fileL,tmp_d);
				if (fileL) free(fileL);
				fileL=tmp_c; tmp_c=NULL;
			} else {
...
static void synchronize_txt(void){
	char *texto="***Synchronization void***\n";
	if (rightC==leftC) return;
#ifdef __GTK_2_0	
  utf8_insert_blue((rightC<leftC)?text_right:text_left,texto,-1);
#else	
	gtk_text_insert (GTK_TEXT ((rightC<leftC)?text_right:text_left),style->font,&filefg,
			&filebg,texto,strlen(texto));
#endif
...
int do_diff(void){
	...
	if (!directive) xfdiff_abort(E_MALLOC);			
	sprintf(directive,"__xfdiff_directive__%s",pidT);
	
	if (!pre_diff()) return FALSE;	

	if (checkdir(fileR) && checkdir(fileL)) {
		if (my_yesno_dialog(_("Include binary files?"))) all_files=TRUE;
		if (my_yesno_dialog(_("Include subdirectories?"))) recursive_patchmake=TRUE;
	}
	addedR=addedL=0;
	...
	if (fork()) {/*  parent will fork to diff  */
		...
		if (fork()==0) {/* forks again, to wait for completion */
			...
			if (patchO) {
				arguments[argc++]="-u";
				arguments[argc++]="-N";
				if (all_files) arguments[argc++]="-a";
				if (recursive_patchmake) arguments[argc++]="-r";
			} else {
				arguments[argc++]="-D";
	...
	/* meanwhile, the first child is busy reading the pipe */
  	gdk_window_set_cursor (diff->window, cursor); gdk_flush ();
#ifdef __GTK_2_0	
...
#endif
	{
		...
		while (1){ 
			...
			if (input[0]=='\n') {
				...
				byrice=0;
				if (binary_line){ /* ignore binary lines */
					if (!binary_section) {
						binary_section=1;
						if (right) {rightC++;writebinary(text_right);}
						if (left)  {leftC++;writebinary(text_left);}
					}
					binary_line=0;
					input=line;
					linecount=0;
...
patchproceed:
	...
	linecount=0; /* bug caught using gtk+1.3*/				
					...
					if (synchronize) synchronize_txt(); 
...
diff_proceed: 
  ...
  utf8_insert(text_left,line,linecount);
			...
			else if (!binary_line) { /* some files have binary data imbedded: */
			...
			if (linecount >= lineL-2) {
				...
				if (binary_line){ /* don't bother with binary lines */
					if (!binary_section) {
						binary_section=1;
						if (right) writebinary(text_right);
						if (left) writebinary(text_left);
					}
					input=line;
					linecount=0;
					continue;
				}
				if (patchO) goto patchproceed;
	...
	if (!patchO) {
		if (!first_diff()) my_show_message(identical_files);
		if (!patching) update_titlesP();
	}
...
applying_loop:
	 ...
	 *    */
	{
		...
		input=line=(char *)malloc(lineL);
		if (!input) xfdiff_abort(E_MALLOC);		
		while (1){ /* patch stderr output: no binary stuff*/
			if (!read(pfd[0],input,1)) break;
			input[1]=0;
			if ((input[0]=='\n')||(strlen(line)>=lineL-2)) {
				...
				show_diag(line);
				input=line;
				if ((strncmp(line,"Reversed",strlen("Reversed"))==0) 
						&& (strstr(line,"detected"))){
	...
	if (reversed_detected) {
		if (my_yesno_dialog(_("Reversed patch detected. Should I try reversing it?"))){ /* new text 1.2.3 */
	...
	while (!feof(Pfile)){ 
		...
		if (input[0]=='\n') {
			...
			if (strncmp(line,"---",3)==0) { 
				...
				strtok(line," ");
				file=strtok(NULL,"\t");
				if (file) {currentF=pushF(file,offset,0);}
				/*printf("%s: offset=%d, length=%d\n",
		...
		input++;
		if (linecount >= lineL-2) { /* by now line is of no interest */
			ricefound=1;
			input=line;
			linecount=0;
			continue;
...
/* first, let's parse the patch file. */
	...
	while (1) {
		currentF=headF;
		if ((fileH=process_patch(currentF))!=2) break;
	}
	update_titlesP();
  	gdk_window_set_cursor (diff->window, NULL);
	return fileH;
...
int main (int argc, char *argv[])
{
	...
	if (argc >= 2) for (i=1;i<argc;i++) {
		...
		if ((strcmp(argv[i],"-h")==0) || (strcmp(argv[i],"--help")==0)) {
			...
			fprintf(stdout,
				_("use xfdiff [ -h | --help | -p] [leftfile] [rightfile]\n"));
			fprintf(stdout,
				_("-p : leftfile and rightfile are a patchdir and a patchfile\n"));
			fprintf(stdout,
				_("-PN: N is strip level (0-9) for patch (quick toggle)\n"));
			fprintf(stdout,
				_("-R: patch file is reversed (quick toggle)\n"));
			fprintf(stdout,
				_("-N: patch file is not reversed (quick toggle)\n"));
			fprintf(stdout,
				_("-h, --help : print this message\n\n"));
			fprintf(stdout,
				_("xfdiff (c) Edscott Wilson Garcia 2001, under GNU-GPL\n"));
			fprintf(stdout,
				_("XFCE modules copyright Olivier Fourdan 1999-2001, under GNU-GPL\n\n"));
			exit(1);
		}
	}
	if (argc >= leftA+1 ) {
		if (patching) fileD = assign(fileD,argv[leftA]);
		else fileL = assign(fileL,argv[leftA]);	
	}	
	if (argc >= rightA+1 ) {
		if (patching) fileP = assign(fileP,argv[rightA]);
		else 	fileR = assign(fileR,argv[rightA]);
	}		
...
/*printf("1...\n");*/
	...
	if (patching){ 
		if ((fileD) && (!checkdir(fileD))) fileD=assign(fileD,NULL);
		if ((fileP) && (checkdir(fileP))) fileP=assign(fileP,NULL);
		if ((fileD)&&(fileP)) do_patch();
	} else { 
		if ((fileR)&&(fileL)) do_diff();
	}
	update_titlesP();
	gtk_main ();
	return (0);
