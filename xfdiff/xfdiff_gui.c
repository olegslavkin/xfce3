...
static void cb_wheel(GtkWidget *widget,GdkEventButton * event, void *data){
	int new_adj;
	
	new_adj=(GTK_ADJUSTMENT (adj)->value);

	if (event->button == 4) new_adj -= (3*lineH); /*printf("button clicked4\n");*/
	if (event->button == 5) new_adj += (3*lineH); /*printf("button clicked5\n");*/
	if (new_adj < 0 )new_adj=0;
		       ...
		       void *client)
{
  ...
  if (strncmp(text,"file:",strlen("file:"))==0){
	  ...
	  if (patching) {
	  	if (checkdir(file)) fileD=assign(fileD,file);
		else  fileP=assign(fileP,file);
	  } else { /* spot 1/3 where fileR, fileL come in */
#ifdef __GTK_2_0	
		  if (entry==viewR){
#else	
		  if (entry==text_right){
...
static void cb_patchapply(GtkWidget *widget,gpointer data){
	...
	applying_patch=0;
	if (undoing) reversed = ! reversed;
	if (!check_reject()) my_show_message((undoing)?_("Patch has been undone."):_("Patch has been applied."));
}
...
static void cb_patchmake(GtkWidget *widget,gpointer data){
	...
	if (!patchO) { cancelled=1; goto exit_patchmake;}
	
	if (fileLD) fileL=assign(fileL,fileLD);
	if (fileRD) fileR=assign(fileR,fileRD);
	
	if (!fileL) fileL=prompt_path(no_left_path,fileL);
	if ((fileL)&&(!fileR)) fileR=prompt_path(no_right_path,fileR);
	if ((!fileL)||(!fileR)) goto exit_patchmake;
	...
	if (!do_diff()) my_show_message(_("Patch file could not be created"));
	else {
		char *number;
		int numL,numR;
...
exit_patchmake:
	if (cancelled) my_show_message(_("Patch file creation cancelled")); 
...
static void cb_radio_strip(GtkWidget *widget, gpointer data){
	...
	if (strip != laststrip) {
		...
		/* reparse to check if null patchfile with that strip: */
	       	cleanF(); 
		update_titlesP();cleanT();cleanP();cleanA();
	}
}
/*
...
static void hideshow_menus(void) {
	gint i;	
	if (patching) {
		for(i = 0; i < STRIP_LEVELS; i++)	{
			...
			if (!autostrip) {
				if (!GTK_WIDGET_VISIBLE (stripM[i])) gtk_widget_show (stripM[i]);
			} else {
				if (GTK_WIDGET_VISIBLE (stripM[i])) gtk_widget_hide (stripM[i]);
			}
		}
...
static void cb_toggle_mode(GtkWidget *widget,gpointer data){
	static char *LfileR=NULL,*LfileL=NULL;
	if ((int)((long) data)) {
		patching=!patching;
		cleanA();cleanP();cleanT();cleanF();
		if (patching) {
			LfileR=assign(LfileR,fileR);
			LfileL=assign(LfileL,fileL);
		} else {
			if (fileRR) remove(fileRR);fileRR=assign(fileRR,NULL);
			if (fileO) remove(fileO);fileO=assign(fileO,NULL);
			if (fileI) remove(fileI);fileI=assign(fileI,NULL);
...
static void cb_toggle_synchronize(GtkWidget *widget,gpointer data){
	synchronize = !synchronize;
	if (silent) show_diag (_("Line synchronization enabled.\n"));
	else show_diag (_("Line synchronization disabled.\n"));
#ifdef AUTOSAVE
	cb_save_defaults(NULL,NULL); 
#endif	
	{
		int old_adj=0;
		polygon *thisP;
		if (current) { old_adj = current->topLR;}
		if (patching) cb_do_patch(NULL,(gpointer)((int) 0)); 
		else cb_do_diff(NULL,(gpointer)((int) 0));
		thisP=head;
		while (thisP) {
			if (thisP->topLR==old_adj) {old_adj=thisP->topR;break;}
			thisP=thisP->next;
		}
		current=thisP;
...
static void cb_toggle_autostrip(GtkWidget *widget,gpointer data){
	autostrip=!autostrip;
	hideshow_menus();
}
...
static void cb_toggle_filledP(GtkWidget *widget,gpointer data){
	...
	else show_diag (_("Polygons will now *not* be filled.\n"));
#ifdef AUTOSAVE
	cb_save_defaults(NULL,NULL); 
#endif
	configure_event(drawA,NULL);	
...
static void cb_select_colors(GtkWidget * widget, gpointer data)
{
  ...
  switch (caso){
	  case 1:
	 	newcolor = xf_colorselect 
		  (_("Select highlight background color"),colors);
		break;
	  default:
	  	newcolor = xf_colorselect 
		  (_("Select highlight text color"),colors);
		break;
  }
#else	
...
static void cb_do_patch(GtkWidget *widget,gpointer data){
	...
	if (!headF) {
		if (!patching) cb_do_diff(widget,data);
		else { do_patch(); check_reject();}
		return;
	}
...
*  the function is deprecated in 2.0 */	
	...
	if (thisF) currentF=thisF;
		...
		char *texto;
		if (!patching) {
			texto=(char *)malloc(strlen(s)
					+strlen(_("Warning: cannot read from "))+2);
			sprintf(texto,"%s %s",_("Warning: cannot read from "),s);
		} else {
			texto=(char *)malloc(strlen(s)
					+strlen(_("is not in patch file."))+2);
	...
	if (patching) process_patch(currentF);
	else {
...
static void cb_do_diff_new(GtkWidget *widget,gpointer data){
	char Pstrip[4];
	char *Pright,*Pleft;
	char *arguments[10]; int argc=0;


	arguments[argc++]="xfdiff";
	if (patching) arguments[argc++]="-p";
	strcpy(Pstrip,"-P"); Pstrip[2] = strip+'0'; Pstrip[3]=0;
	arguments[argc++]=Pstrip;
	if (reversed) arguments[argc++]="-R"; else arguments[argc++]="-N"; 
	if (patching) {
		arguments[argc++]="-p";
		Pleft=fileD;
		Pright=fileP;
	} else {
		if (fileRD) Pright=fileRD; else Pright=fileR; 
		if (fileLD) Pleft=fileLD; else Pleft=fileL; 
	}
	if (Pleft) arguments[argc++]=Pleft;
	if ((Pright)&&(Pleft)) arguments[argc++]=Pright;
...
void cb_next_diff(GtkWidget *widget,gpointer data){
	...
	if (!current) value=0;
		...
		if (next) {
			...
			else {my_show_message(_("There is no next difference"));}
		} else {
			if (current->previous)current=current->previous;
			else {my_show_message(_("There is no previous difference"));}
		}
...
static void cb_next_file(GtkWidget *widget,gpointer data){
	long next;
	next = (long) data;
	if (currentF==NULL)  {my_show_message(_("There is no such file"));return;}
	
	if (next) {
		if (currentF->next) currentF=currentF->next;
		else {my_show_message(_("There is no next file"));return;}
		
	} else {
		if (currentF->previous) currentF=currentF->previous;
		else {my_show_message(_("There is no previous file"));return;}		
	}
...
static void cb_get_fileR (GtkWidget * widget,gpointer data)
{
...
static void cb_get_fileL (GtkWidget * widget,gpointer data)
{
...
static void cb_get_fileD (GtkWidget * widget,gpointer data)
{
	fileD = get_the_file(fileD);
	/* checknotdir() includes my_show_message */
	if (checknotdir(fileD)) fileD=assign(fileD,NULL);
 	update_titlesP();
	
}

static void cb_get_fileP (GtkWidget * widget,gpointer data)
{
...
static GtkWidget * create_menu (GtkWidget * top)
{
  ...
  for (i=0;i<PATCH_MENUS;i++) patchM[i]=NULL;
  for (i=0;i<DIFF_MENUS;i++) diffM[i]=NULL;
  /* Create "File" menu */
  menu = shortcut_menu(MENUBAR,menubar,_("File"),NULL,NULL);
  diffM[0]=shortcut_menu(SUBMENU,menu,_("Select left file"),
		  (gpointer)cb_get_fileL, NULL);
  diffM[1]=shortcut_menu(SUBMENU,menu,_("Select right file"),
		  (gpointer)cb_get_fileR, NULL);
  patchM[0]=shortcut_menu(SUBMENU,menu,_("Select patch file"),
		  (gpointer)cb_get_fileP, NULL);
  patchM[1]=shortcut_menu(SUBMENU,menu,_("Select directory where to apply patch"),
...
/*  shortcut_menu(TOGGLENOT,menu,_("Show diagnostics"),
  ...
  patchM[3]=shortcut_menu(TOGGLE,menu,_("Assume patch is reversed"),
...
#endif
		if (!autostrip) gtk_widget_show (stripM[i]);
 		gtk_signal_connect (GTK_OBJECT (stripM[i]),"activate",
  ...
  diffM[4]=shortcut_menu(SUBMENU,menu,_("View differences"),
		  (gpointer)cb_do_diff, (gpointer)((int) 1));
  diffM[5]=shortcut_menu(SUBMENU,menu,_("View differences in new"),
  ...
  diffM[7]=shortcut_menu(SUBMENU,menu,_("Create patch file"),
...
/*  gtk_box_pack_start (GTK_BOX (vbox),bbox ,TRUE, TRUE, 0);*/
  ...
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (scrolled_windowL),
  ...
  gtk_text_view_set_text_window_size 
	  ((GtkTextView *)viewR,sizeL*lineW,sizeH*lineH);
...
/*  bbox = gtk_hbutton_box_new ();*/
 ...
 /* gtk_button_box_set_spacing (GTK_BUTTON_BOX (bbox), 5);*/
  ...
  button = shortcut_button(bbox,_("Clear"),
             (gpointer)on_clear_show_diag,(gpointer) text);
...
/*  gtk_button_new_with_label (_("Clear"));
  ...
  button = shortcut_button(bbox,_("Next difference"),(gpointer)cb_next_diff,
		  (gpointer) ((long) 1));
  gtk_widget_grab_default (button);

  button = shortcut_button(bbox,_("Prev difference"),(gpointer)cb_next_diff,
		  (gpointer) ((long) 0));
  button = shortcut_button(bbox,_("Next file"),(gpointer)cb_next_file,
		   (gpointer) ((long) 1)); 
  button = shortcut_button(bbox,_("Previous file"),(gpointer)cb_next_file,
		   (gpointer) ((long) 0));
 ...
 // gtk_widget_set_events (drawA,GDK_EXPOSURE_MASK);
  ...
  gtk_drag_dest_set (viewL, GTK_DEST_DEFAULT_ALL,target_table, NUM_TARGETS, 
		  GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK);
  gtk_drag_dest_set (viewR, GTK_DEST_DEFAULT_ALL,target_table, NUM_TARGETS, 
		  GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK);

  gtk_signal_connect (GTK_OBJECT (viewL), "drag_data_received",
			  GTK_SIGNAL_FUNC (on_drag_data_received), 
                          (gpointer) NULL);
  gtk_signal_connect (GTK_OBJECT (viewR), "drag_data_received",
			  GTK_SIGNAL_FUNC (on_drag_data_received), 
                          (gpointer) NULL);
	...
	/* this hack doesnt work with gtk+1.3 */	
#else	  
  gtk_signal_connect (GTK_OBJECT (viewR), "button_press_event",
			  GTK_SIGNAL_FUNC (cb_wheel), 
                          (gpointer) NULL);
  gtk_signal_connect (GTK_OBJECT (viewL), "button_press_event",
			  GTK_SIGNAL_FUNC (cb_wheel), 
                          (gpointer) NULL);
#endif
