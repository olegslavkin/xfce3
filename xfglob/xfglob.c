...
static GdkPixmap *gPIX_block_dev, *gPIX_char_dev, *gPIX_dir_close,
  *gPIX_go_to, *gPIX_go_down, *gPIX_exe, *gPIX_sexe, *gPIX_fifo, *gPIX_page,
  *gPIX_page_lnk, *gPIX_socket, *gPIX_help;
static GdkBitmap *gPIM_block_dev, *gPIM_char_dev, *gPIM_dir_close,
  *gPIM_go_to, *gPIM_go_down, *gPIM_exe, *gPIM_sexe, *gPIM_fifo, *gPIM_page,
  *gPIM_page_lnk, *gPIM_socket, *gPIM_help;
...
static GtkWidget *regexpScroll, *findTree, *filterScroll,*xdevScroll,
                 *permScroll,*userScroll,*sizeScroll,*timeScroll;
static GList *itemsP = NULL, *itemsF = NULL, *itemsG = NULL;

static gboolean considerTime = FALSE, considerSize = FALSE,
		  ...
		  guint info, guint time, gpointer data)
{
  ...
  if (!(GTK_CLIST (ctree)->selection)) {
	...
	{
	  node = selection->data;
          if ((!gtk_ctree_node_get_text (ctree, node, 2, path))||
 		(!gtk_ctree_node_get_text (ctree, node, 1, path+1)))		  
	  {
		  return;
	  }
	  if (path[0][0]=='/') {
      ...
      while (selection) 
	{
      ...
      gtk_selection_data_set (selection_data,
			      selection_data->target, 8, dnd_data, len);
}


static void
on_drag_data_received (GtkWidget * entry, GdkDragContext * context,
		       gint x, gint y,
  ...
  if (strncmp (text, "file:", strlen ("file:")) == 0)
	...
	for (i = 0; i < strlen (file); i++)
      ...
      gtk_entry_set_text (GTK_ENTRY
			  (GTK_COMBO (find_options.path_entry)->entry), file);
...
void
set_widget (int pack, int kind, char *name, GtkWidget * widget,
...
abort_glob (GtkWidget * widget, gpointer data)
{
  if (Gpid)
      ...
      if (data)
	{
	  char *message;
	  message =
	    (char *) malloc (strlen (_("Results limit reached")) + 64);
	  sprintf (message, "%s (%d)\n", _("Results limit reached"),
		   fileLimit);
	  show_cat (message);
...
{
 if (!cat) show_cat(_("Find results.\n"));
...
/*  button = gtk_button_new ();*/
  if (togg) button = gtk_toggle_button_new ();
...
show_cat (char *message)
{
  ...
  vpaned = gtk_vpaned_new ();
  {
    ...
    gtk_box_pack_start (GTK_BOX (GTK_DIALOG (cat)->vbox), vpaned, TRUE, TRUE,
			0);
	...
	gtk_ctree_set_expander_style (GTK_CTREE (findTree),
	...
	gtk_clist_set_selection_mode (GTK_CLIST (findTree), GTK_SELECTION_EXTENDED);
        gtk_signal_connect (GTK_OBJECT (findTree), "drag_data_get",
		      GTK_SIGNAL_FUNC (on_drag_data_get), NULL);
  	gtk_signal_connect (GTK_OBJECT (findTree), "click_column",
		      GTK_SIGNAL_FUNC (on_click_column), NULL);
	gtk_drag_source_set (findTree,
		       GDK_BUTTON1_MASK | GDK_BUTTON2_MASK |
		       GDK_BUTTON3_MASK, target_table, NUM_TARGETS,
  ...
  gtk_widget_show (button);
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      GTK_SIGNAL_FUNC (raiseG_cb),NULL);
...
GtkWidget *create_help(GtkWidget *parent,char *help_text)
{
   GtkWidget *scroll,*helpText;
   
   scroll = gtk_scrolled_window_new (NULL, NULL);
   set_widget (PACK, SCROLLED, "scrolled_window_filter",
	scroll, parent, find, 0, NULL);
   if (GTK_WIDGET_VISIBLE (scroll)) gtk_widget_hide (scroll);
    
   helpText = gtk_label_new (help_text);
   gtk_label_set_justify (GTK_LABEL (helpText), GTK_JUSTIFY_LEFT);
   gtk_scrolled_window_add_with_viewport
        ((GtkScrolledWindow *)scroll,helpText);
...
toggle_view (GtkWidget * widget, gpointer data)
{
  ...
  i = (int) ((long) (data));
  switch (i) {
    case 0: adjust = options_adj; considerWhat = &considerUser; break;
    case 1: adjust = options_adj; considerWhat = &considerSize; break;
    case 2: adjust = options_adj; considerWhat = &considerTime; break;
    case 3: adjust = find_adj; considerWhat = NULL; break;
    case 5: adjust = options_adj; considerWhat = &considerPerm; break;
    default: adjust = NULL; considerWhat = NULL; break;
  }
  switch (i)
  {
      case 0: box = UserVbox; break;
      case 1: box = SizeVbox; break;
      case 2: box = TimeVbox; break;
      case 3: box = regexpScroll; break;
      case 4: box = filterScroll; break;
      case 5: box = PermVbox; break;
      case 6: box = permScroll; break;
      case 7: box = userScroll; break;
      case 8: box = sizeScroll; break;
      case 9: box = timeScroll; break;
      case 10:box = xdevScroll; break;
      default: return;
  }
...
static void
make_toggle (gchar * label, gchar * name, GtkWidget * parent,
	     const *flag, gboolean state)
{
  GtkWidget *button;
  button = gtk_check_button_new_with_label (label);
  set_widget (PACK, BUTTON, name, button, parent, find, 2, NULL);
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      GTK_SIGNAL_FUNC (toggle_option), (gpointer) flag);
...
make_radio1 (gchar * label, gchar * name, GtkWidget * parent, const *flag)
{
  ...
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      GTK_SIGNAL_FUNC (toggle_radio1), (gpointer) flag);
  set_widget (PACK, BUTTON, name, button, parent, find, 2, NULL);
}
static void
make_radio2 (gchar * label, gchar * name, GtkWidget * parent, const *flag)
{
  ...
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      GTK_SIGNAL_FUNC (toggle_radio2), (gpointer) flag);
  set_widget (PACK, BUTTON, name, button, parent, find, 2, NULL);
}
static void
make_radio3 (gchar * label, gchar * name, GtkWidget * parent, const *flag)
{
  ...
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      GTK_SIGNAL_FUNC (toggle_radio3), (gpointer) flag);
  set_widget (PACK, BUTTON, name, button, parent, find, 2, NULL);
}
...
gint
itemcompare (gconstpointer a, gconstpointer b)
{
...
/* limit */
  fileLimit =
    gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON (find_options.FindLimit));
...
/* the rest */
  ...
  itemsF =
    put_item (itemsF, filter, (GtkCombo *) (find_options.filter_entry));
  ...
  /* select list */
  s =
    gtk_entry_get_text (GTK_ENTRY (GTK_COMBO (find_options.filetype)->entry));
...
/* obsolete: replace with -o option
  ...
  if (considerTime) { 
   if (options & MTIME) argument[i++] = "-M";
   if (options & ATIME) argument[i++] = "-A";
   if (options & CTIME) argument[i++] = "-C";
   if (hours > 0)
     {
       if (options & MINUTES) argument[i++] = "-k";
       if (options & HOURS) argument[i++] = "-h";
       if (options & DAYS) argument[i++] = "-d";
       if (options & MONTHS) argument[i++] = "-m";
   ...
   gtk_timeout_add (2080, (GtkFunction) GlobWait, (gpointer)((long) childPID));
  {
    ...
    findNode=gtk_ctree_insert_node ((GtkCTree *) findTree, 
		    NULL, NULL, textos, 3, 
		    gPIX_go_to, gPIM_go_to, gPIX_go_down, gPIM_go_down, 
		    FALSE, TRUE);	/* isleaf,expand */
    {
	    int *data;
	    data=(int *)malloc(3*sizeof(int));
	    data[0]=data[1]=data[2]=0;
    	    gtk_ctree_node_set_row_data_full ((GtkCTree *)findTree,findNode, 
					data, node_destroy);
    }
...
void jam(char *file,GtkOptionMenu *optmen,gpointer func){
	FILE *archie;
	char line[256];
	...
	while (!feof(archie) && (fgets(line,255,archie)))
	  {
	    ...
	    gtk_menu_append (GTK_MENU (optionmenu), menuitem);
	    gtk_signal_connect (GTK_OBJECT (menuitem),"activate", 
			    GTK_SIGNAL_FUNC (func),(gpointer) t);
	  }
...
get_path(GtkWidget *widget,gpointer data) {
	...
	if (fileS) {
	    gtk_entry_set_text (GTK_ENTRY
		(GTK_COMBO (find_options.path_entry)->
		 entry), fileS);
  	    gtk_widget_set_usize (find_options.path_entry,-1,-1);
...
create_find (char *path)
{
  ...
  ftypes[0] = _("Any kind"),
  ftypes[1] = _("Regular"),
    ftypes[2] = _("Directory"),
    ftypes[3] = _("Symlink"),
    ftypes[4] = _("Socket"),
    ftypes[5] = _("Block device"),
    ftypes[6] = _("Character device"),
    ftypes[7] = _("FIFO"), 
    ftypes[8] = NULL;
...
/* first circle : main window  */
  ...
  gPIX_go_down =
    MyCreateGdkPixmapFromData (go_down_xpm, find, &gPIM_go_down, FALSE);
  gPIX_go_to =
    MyCreateGdkPixmapFromData (go_to_xpm, find, &gPIM_go_to, FALSE);
  gPIX_block_dev =
    MyCreateGdkPixmapFromData (block_dev_xpm, find, &gPIM_block_dev, FALSE);
  gPIX_char_dev =
    MyCreateGdkPixmapFromData (char_dev_xpm, find, &gPIM_char_dev, FALSE);
  gPIX_dir_close =
    MyCreateGdkPixmapFromData (dir_close_xpm, find, &gPIM_dir_close, FALSE);
  ...
  gPIX_page_lnk =
    MyCreateGdkPixmapFromData (page_lnk_xpm, find, &gPIM_page_lnk, FALSE);
  gPIX_socket =
    MyCreateGdkPixmapFromData (socket_xpm, find, &gPIM_socket, FALSE);
...
/* fourth circle: notebook*/
	...
	set_widget (ADD, NOTEBOOK, "find_notebook", find_notebook, note_vbox,
		    find, 0, NULL);
...
/* fifth circle : scrolled window */
	{
	  ...
	  set_widget (ADD, SCROLLED, "scrolled_window1", scrolled_window,
		      find_notebook, find, 5, NULL);
	  find_adj =
...
/* sixth circle : vbox */
	  {
	    ...
	    set_widget (SCROLL, BOX, "page_vbox", page_vbox, scrolled_window,
			find, 5, NULL);
/* seventh circle : frame  */
	    {
	      upframe = gtk_frame_new (_("Find"));
	      set_widget (PACK, FRAME, "find_upframe", upframe, page_vbox,
			  find, 5, NULL);
/* eigth circle : vbox  */
	      {
		vbox = gtk_vbox_new (FALSE, 0);
		set_widget (ADD, BOX, "find_vbox", vbox, upframe, find, 5,
			    NULL);
/* ninth circle : hboxes */
		{
/* path box */
		  hbox1 = gtk_hbox_new (FALSE, 0);
		  set_widget (ADD, BOX, "path_hbox", hbox1, vbox, find, 5,
			      NULL);

		  label = gtk_label_new (_("Path : "));
		  set_widget (PACK, LABEL, "path_label", label, hbox1, find,
			      5, NULL);

		  find_options.path_entry = gtk_combo_new ();
		  set_widget (PACK, COMBO, "find_options.path_entry",
			      find_options.path_entry, hbox1, find, 5, NULL);
		  if (path)
		    gtk_entry_set_text (GTK_ENTRY
					(GTK_COMBO (find_options.path_entry)->
					 entry), path);
		  gtk_combo_disable_activate ((GtkCombo *) find_options.
					      path_entry);
		  gtk_signal_connect (GTK_OBJECT
				      (GTK_COMBO (find_options.path_entry)->
				       entry), "activate",
				      GTK_SIGNAL_FUNC (find_ok_cb), NULL);
		  button=icon_button (0,dir_open_xpm, _("Browse"));
		  gtk_box_pack_end (GTK_BOX (hbox1), button, FALSE, FALSE, 0);
		  gtk_signal_connect (GTK_OBJECT (button), "clicked",
				      GTK_SIGNAL_FUNC (get_path), NULL);
		  gtk_widget_show (button);

/* Filter box : */
		  hbox2 = gtk_hbox_new (FALSE, 0);
		  set_widget (ADD, BOX, "filter_hbox", hbox2, vbox, find, 5,
			      NULL);

		  label = gtk_label_new (_("File filter : "));
		  set_widget (PACK, LABEL, "filter_label", label, hbox2, find,
			      5, NULL);

		  find_options.filter_entry = gtk_combo_new ();
		  set_widget (PACK, COMBO, "find_options.filter_entry",
			      find_options.filter_entry, hbox2, find, 5,
			      NULL);
		  gtk_combo_disable_activate ((GtkCombo *) find_options.
					      filter_entry);
		  gtk_signal_connect (GTK_OBJECT
				      (GTK_COMBO (find_options.filter_entry)->
				       entry), "activate",
				      GTK_SIGNAL_FUNC (find_ok_cb), NULL);
		  ...
		  gtk_signal_connect (GTK_OBJECT (button), "clicked",
				      GTK_SIGNAL_FUNC (toggle_view),
				      (gpointer) ((long) 4));
		  gtk_widget_show (button);
		 
		  if (DEFAULT_FILTER)
		    gtk_entry_set_text (GTK_ENTRY (find_options.filter_entry),
					DEFAULT_FILTER);
		  filterScroll=create_help(vbox, N_
			("Basic rules:\n"
		       "\n"
		  ...
		  hbox2 = gtk_hbox_new (FALSE, 0);
		  set_widget (PACK, BOX, "recursive_hbox2", hbox2, vbox, find,
			      5, NULL);

		  make_toggle (_("Search subdirectories too"),
			       "find_options.recursive", hbox2, &RECURSIVE,
			       TRUE);

		  label = gtk_label_new (_("Limit results to first : "));
		  set_widget (PACK, LABEL, "limit_label", label, hbox2, find,
			      5, NULL);
		  ...
		  set_widget (PACK, SPINBUTTON, "find_options.FindLimit",
			      find_options.FindLimit, hbox2, find, 5, NULL);
/****/
		}		/* end - circle 9 */
	      }			/* end - circle 8 */
	    }			/* end - circle 7 */
/* seventh circle : frame */
	    {
	      ...
	      set_widget (PACK, FRAME, "grep_frame", upframe, page_vbox, find,
			  5, NULL);
/* eighth circle : vbox  */
	      {
		vbox = gtk_vbox_new (FALSE, 0);
		set_widget (ADD, BOX, "grep_vbox", vbox, upframe, find, 5,
			    NULL);
/* ninth circle : hboxes */
		{
/* grep_hbox1 */
		  hbox1 = gtk_hbox_new (FALSE, 0);
		  set_widget (PACK, BOX, "grep_hbox1", hbox1, vbox, find, 5,
			      NULL);

		  label = gtk_label_new (_("Containing : "));
		  set_widget (PACK, LABEL, "grep_label", label, hbox1, find,
			      5, NULL);

		  find_options.grep_entry = gtk_combo_new ();
		  set_widget (PACK, COMBO, "find_options.grep_entry",
			      find_options.grep_entry, hbox1, find, 5, NULL);
		  gtk_combo_disable_activate ((GtkCombo *) find_options.
					      grep_entry);
		  gtk_signal_connect (GTK_OBJECT
				      (GTK_COMBO (find_options.grep_entry)->
				       entry), "activate",
				      GTK_SIGNAL_FUNC (find_ok_cb), NULL);
/* grep_hbox2 */
		  hbox2 = gtk_hbox_new (FALSE, 0);
		  set_widget (PACK, BOX, "grep_hbox2", hbox2, vbox, find, 5,
			      NULL);

		  make_toggle (_("Case sensitive search"),
			       "find_options.casesense", hbox2,
			       &CASE_SENSITIVE, FALSE);
		  make_toggle (_("Extended regexp"), "find_options.regexp", hbox2,
			       &REGEXP, FALSE);
		  ...
		  gtk_signal_connect (GTK_OBJECT (button), "clicked",
				      ...
				      (gpointer) ((long) 3));
		  gtk_widget_show (button);
		  regexpScroll=create_help(vbox, N_
...
/* fifth circle : scrolled window */
	{
	  scrolled_window = gtk_scrolled_window_new (NULL, NULL);
	  set_widget (ADD, SCROLLED, "scrolled_window2", scrolled_window,
		      find_notebook, find, 5, NULL);
	  options_adj =
	    gtk_scrolled_window_get_vadjustment ((GtkScrolledWindow *)
						 scrolled_window);
/* sixth circle : vbox */
	  {
	    page_vbox = gtk_vbox_new (FALSE, 0);
	    set_widget (SCROLL, BOX, "page_vbox2", page_vbox, scrolled_window,
			find, 5, NULL);
/* seventh circle : frame  */
	    {
	      upframe = gtk_frame_new (_("File considerations"));
	      set_widget (PACK, FRAME, "upframe3", upframe, page_vbox, find,
			  5, NULL);
/* eigth circle : vbox  */
	      {
		vbox = gtk_vbox_new (FALSE, 0);
...
/* file-type box */
		  ...
		  set_widget (PACK, LABEL, "Ftype_label", label, hbox1, find,
			      5, NULL);

		  find_options.filetype = gtk_combo_new ();
		  set_widget (PACK, COMBO, "Ftypes", find_options.filetype,
			      hbox1, find, 5, list_ftypes);
...
/* -xdev box */
		  ...
		  make_toggle (_("Stay on single filesystem"), "find_options.xdev", hbox2,
			       &XDEV, FALSE);
		  ...
		  gtk_signal_connect (GTK_OBJECT (button), "clicked",
				      ...
				      (gpointer) ((long) 10));
		  gtk_widget_show (button);
		  xdevScroll=create_help(vbox, N_
...
/* seventh circle : frame  */
	    {
	      ...
	      set_widget (PACK, FRAME, "upframe1", upframe, page_vbox, find,
			  5, NULL);
...
/* perm stuff */
		...
		button =
		  gtk_check_button_new_with_label (_("Permission considerations"));
		set_widget (PACK, BUTTON, "toggleSize", button, hbox, find, 0,
			    NULL);
		gtk_signal_connect (GTK_OBJECT (button), "clicked",
				    GTK_SIGNAL_FUNC (toggle_view),
				    (gpointer) ((long) 5));
		...
		gtk_signal_connect (GTK_OBJECT (button), "clicked",
				      ...
				      (gpointer) ((long) 6));
		gtk_widget_show (button);
		permScroll=create_help(vbox, N_
		...
		set_widget (PACK, BOX, "vbox2x", PermVbox, vbox, find, 0,
			    NULL);
/* ninth circle : hboxes */
		{
/*  */
		  hbox1 = gtk_hbox_new (FALSE, 0);
		  set_widget (PACK, BOX, "hbox1y", hbox1, PermVbox, find, 5,
			      NULL);
...
/* user/group stuff */
		...
		set_widget (ADD, BOX, "hbox", hbox, vbox, find, 5, NULL);
		button =
		  gtk_check_button_new_with_label (_("User/group considerations"));
		set_widget (PACK, BUTTON, "toggleSize", button, hbox, find, 0,
			    NULL);
		gtk_signal_connect (GTK_OBJECT (button), "clicked",
				    GTK_SIGNAL_FUNC (toggle_view),
		...
		gtk_signal_connect (GTK_OBJECT (button), "clicked",
				      ...
				      (gpointer) ((long) 7));
		gtk_widget_show (button);
		userScroll=create_help(vbox, N_
		...
		set_widget (PACK, BOX, "vbox2", UserVbox, vbox, find, 0,
			    NULL);
/* ninth circle : hboxes */
		{
/* user box */
		  hbox1 = gtk_hbox_new (FALSE, 0);
		  set_widget (PACK, BOX, "hbox1", hbox1, UserVbox, find, 5,
			      NULL);

		  label = gtk_label_new (_("User ID: "));
		  set_widget (PACK, LABEL, "U_label", label, hbox1, find,
			      5, NULL);
		  
		  button = gtk_option_menu_new ();
		  set_widget (PACK, BUTTON, "find_options.user_id",
			      button, hbox1, find, 5, NULL);
		  jam("/etc/passwd",(GtkOptionMenu *)button,GTK_SIGNAL_FUNC (cb_user));
		  find_options.user_id=NULL;
		  
		  hbox2 = gtk_hbox_new (FALSE, 0);
		  set_widget (PACK, BOX, "hbox2", hbox2, UserVbox, find, 5,
			      NULL);

		  label = gtk_label_new (_("Group ID: "));
		  set_widget (PACK, LABEL, "G_label", label, hbox2, find,
			      5, NULL);   		  button = gtk_option_menu_new ();
		  set_widget (PACK, BUTTON, "find_options.group_id",
			      button, hbox2, find, 5, NULL);
...
/* size stuff */
		...
		set_widget (ADD, BOX, "hbox", hbox, vbox, find, 5, NULL);
		button =
		  gtk_check_button_new_with_label (_("Size considerations"));
		set_widget (PACK, BUTTON, "toggleSize", button, hbox, find, 0,
			    NULL);
		gtk_signal_connect (GTK_OBJECT (button), "clicked",
				    GTK_SIGNAL_FUNC (toggle_view),
		...
		gtk_signal_connect (GTK_OBJECT (button), "clicked",
				      ...
				      (gpointer) ((long) 8));
		gtk_widget_show (button);
		sizeScroll=create_help(vbox, N_
		...
		set_widget (PACK, BOX, "vbox2", SizeVbox, vbox, find, 0,
			    NULL);
/* ninth circle : hboxes */
		{
/* sizeG box */
		  ...
		  hbox1 = gtk_hbox_new (FALSE, 0);
		  set_widget (PACK, BOX, "hbox1", hbox1, SizeVbox, find, 5,
			      NULL);

		  label = gtk_label_new (_("Size greater than (KB): "));
		  set_widget (PACK, LABEL, "SizeM_label", label, hbox1, find,
			      5, NULL);
		  ...
		  set_widget (PACK, SPINBUTTON, "find_options.sizeG",
			      find_options.sizeG, hbox1, find, 5, NULL);
/* sizeM box */
		  hbox2 = gtk_hbox_new (FALSE, 0);
		  set_widget (PACK, BOX, "hbox2", hbox2, SizeVbox, find, 5,
			      NULL);

		  label = gtk_label_new (_("Size less than (KB): "));
		  set_widget (PACK, LABEL, "SizeM_label", label, hbox2, find,
			      5, NULL);

		  Fa = gtk_adjustment_new (0, 0, 1024 * 1024 * 1024, 1, 1024, 1024);
		  find_options.sizeM =
		    gtk_spin_button_new (GTK_ADJUSTMENT (Fa), 1, 0);
		...
		/*  gtk_signal_connect (GTK_OBJECT (find_options.sizeM), "changed",
		  ...
		  set_widget (PACK, SPINBUTTON, "find_options.sizeM",
			      find_options.sizeM, hbox2, find, 5, NULL);
		...
		set_widget (ADD, BOX, "hbox", hbox, vbox, find, 5, NULL);
		button =
		  gtk_check_button_new_with_label (_("Time considerations"));
		set_widget (PACK, BUTTON, "toggleTime", button, hbox, find, 0,
			    NULL);
		gtk_signal_connect (GTK_OBJECT (button), "clicked",
				    GTK_SIGNAL_FUNC (toggle_view),
				    (gpointer) ((long) 2));
		...
		gtk_signal_connect (GTK_OBJECT (button), "clicked",
				      ...
				      (gpointer) ((long) 9));
		gtk_widget_show (button);
		timeScroll=create_help(vbox, N_
		...
		set_widget (PACK, BOX, "vbox2", TimeVbox, vbox, find, 0,
			    NULL);
/* ninth circle : hboxes */
...
/* hours box */
		  ...
		  hbox1 = gtk_hbox_new (FALSE, 0);
		  set_widget (PACK, BOX, "hbox3", hbox1, TimeVbox, find, 5,
			      NULL);

		  label =
		    gtk_label_new (_
				   ("In the previous : "));
		  set_widget (PACK, LABEL, "h_label", label, hbox1, find,
			      5, NULL);

		  Fa = gtk_adjustment_new (0, 0, 72, 1, 4, 4);
		  find_options.hours = gtk_spin_button_new (GTK_ADJUSTMENT (Fa),
					 1, 0);
		  set_widget (PACK, SPINBUTTON, "find_options.hours",
			      find_options.hours, hbox1, find, 5, NULL);
...
/* ninth circle : checkbuttons */
		{
		  make_toggle (_("Do not look into binary files"),
			       "find_options.I", vbox, &NOBINARIES, TRUE);

		  make_toggle (_("Output count of matching lines"),
			       "find_options.count", vbox, &COUNT, TRUE);

		  make_radio2 (_("Match anywhere"), "find_options.match",
			       vbox, &MATCH);
		  make_radio2 (_("Match whole words only"),
			       "find_options.words", vbox, &WORDS);
		  make_radio2 (_("Match whole lines only"),
			       "find_options.lines", vbox, &LINES);
		  make_radio2 (_("Output files where no match is found"),
			       "find_options.invert", vbox, &INVERT);
...
/* about */	
	{
	  scrolled_window = gtk_scrolled_window_new (NULL, NULL);
	  set_widget (ADD, SCROLLED, "scrolled_window4", scrolled_window,
		      find_notebook, find, 5, NULL);
	  label =gtk_label_new (_(COPYRIGHT));
	  gtk_scrolled_window_add_with_viewport
		  ((GtkScrolledWindow *)scrolled_window,label);
...
/* fourth circle : notebook tabs stuff */
	...
	set_widget (TAB, TAB_LABEL, "find_tab_label", label, find_notebook,
		    find, 0, NULL);

	label = gtk_label_new (_("Filter options"));
	set_widget (TAB, TAB_LABEL, "glob_label", label, find_notebook, find,
		    1, NULL);

	label = gtk_label_new (_("Content options"));
	set_widget (TAB, TAB_LABEL, "grep_label", label, find_notebook, find,
		    2, NULL);

	label = gtk_label_new (_("About"));
	set_widget (TAB, TAB_LABEL, "about_label", label, find_notebook, find,
		    3, NULL);
      }				/* end - circle 4 */
...
/* fourth circle:  bottom frame */
	...
	set_widget (REPACK, FRAME, "mainframe", bottomframe, note_vbox, find,
		    5, NULL);
/* fifth circle : hbox and contents */
	{
	  hbox = gtk_hbutton_box_new ();
	  set_widget (ADD, BOX, "bottomframe1", hbox, bottomframe, find, 0,
		      NULL);

/* visible part */
	  bottom_hbox1 = gtk_hbutton_box_new ();
	  set_widget (PACK, BOX, "bottomframe1", bottom_hbox1, hbox, find, 0,
		      NULL);

	  button = gtk_button_new_with_label (_("Find"));
	  set_widget (PACK, BUTTON, "find_button", button,
		      bottom_hbox1, find, 5, NULL);
	  GTK_WIDGET_SET_FLAGS (button, GTK_CAN_DEFAULT);
	  gtk_widget_grab_default (button);
          gtk_signal_connect (GTK_OBJECT (button), "clicked",
	  ...
	  set_widget (PACK, BUTTON, "raiseR_button", button,
		      bottom_hbox1, find, 5, NULL);
	  GTK_WIDGET_SET_FLAGS (button, GTK_CAN_DEFAULT);
	  gtk_widget_grab_default (button);
          gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      GTK_SIGNAL_FUNC (raiseR_cb), (gpointer) NULL);

	  button = gtk_button_new_with_label (_("Quit"));
	  set_widget (PACK, BUTTON, "cancel_button",
		      button, bottom_hbox1, find, 5,
		      NULL);
	  GTK_WIDGET_SET_FLAGS (button, GTK_CAN_DEFAULT);
          gtk_signal_connect (GTK_OBJECT (button), "clicked",
...
/* invisible part: */
	  ...
	  set_widget (PACK, BOX, "bottomframe2", bottom_hbox2, hbox, find, 0,
		      NULL);

	  label = gtk_label_new (_("working..."));
	  set_widget (PACK, LABEL, "abort_tab_label", label, bottom_hbox2,
		      find, 0, NULL);

	  button =
	    gtk_button_new_with_label (_("Cancel Search"));
	  set_widget (PACK, BUTTON, "abort_button", button,
		      bottom_hbox2, find, 5, NULL);
	  GTK_WIDGET_SET_FLAGS (button, GTK_CAN_DEFAULT);
          gtk_signal_connect (GTK_OBJECT (button), "clicked",
...
/* and finally, the signals */
  gtk_drag_dest_set (find, GTK_DEST_DEFAULT_ALL,
		     target_table, NUM_TARGETS,
		     GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK);
  gtk_signal_connect (GTK_OBJECT (find),
		      "drag_data_received",
      ...
      if (buffer[0] == '\n')
	{
	  ...
	  if (line[0] == '/')
	      ...
	      else
		{
		  ...
		  data[1]=data[2]=0;
		  if (linecount)
		    sprintf (cuenta, "%d (%s %s)", findCount, linecount,
			     _("lines"));
		  else
		  ...
		  }
		  {
			...
			if (!strlen(path)) textos[2]="/";
		  	node=gtk_ctree_insert_node ((GtkCTree *) findTree, 
				  findNode, NULL, textos, 3, 
				  gPIX, gPIM, NULL, NULL, 
				  TRUE, FALSE);	/* isleaf,expand */
		  	gtk_ctree_node_set_row_data_full ((GtkCTree *)findTree, node, 
					data, node_destroy);
		  }
		  /* path was zapped to directory 5 lines above */
		  show_cat (path); show_cat ("/");  show_cat (filename); show_cat ("\n");
		}
