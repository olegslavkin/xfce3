...
setroot (char *passedstr, char *tiled)
{
  ...
  if (my_strncasecmp (str, NOBACK, strlen (NOBACK)) && existfile (str))
      ...
      if (im)
	{
	  ...
	  else if  ((im->rgb_width > (getdisplaywidth () / 2)) && 
                    (im->rgb_height > (getdisplayheight () / 2))) 
	    {
	      pix_width  = getdisplaywidth ();
	      pix_height = getdisplayheight ();
	    }
      ...
      if (Pixbuf)
	{
	  ...
	  else if ((gdk_pixbuf_get_width (Pixbuf) > (getdisplaywidth () / 2)) &&
	           (gdk_pixbuf_get_height (Pixbuf) > (getdisplayheight () / 2)))
	    {
	      pix_width  = getdisplaywidth ();
	      pix_height = getdisplayheight ();
	    }
	  ...
	  temp = gdk_pixbuf_scale_simple (Pixbuf, pix_width,
			  	          pix_height, GDK_INTERP_BILINEAR);
	...
	return 0;
      ...
      XGetWindowAttributes (tmpDpy, GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()),
			    &root_attr);
      xpm_attributes.colormap = root_attr.colormap;
      xpm_attributes.valuemask = XpmSize | XpmReturnPixels | XpmColormap;
      if (
	  (val =
	   XpmReadFileToPixmap (tmpDpy,
				GDK_WINDOW_XWINDOW (GDK_ROOT_PARENT ()), str,
				&rootXpm, &shapeMask,
				&xpm_attributes)) != XpmSuccess)
	{
      ...
      XSetWindowBackgroundPixmap (tmpDpy,
...
display_back (char *str)
{
  ...
  if ((str)
      && (my_strncasecmp (str, NOBACK, strlen (NOBACK)) && existfile (str))){
...
create_xfbd ()
{
  ...
  gtk_object_set_data (GTK_OBJECT (xfbd), "preview_pixmap_frame",
		       preview_pixmap_frame);
  ...
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (setup_tiled),
				(!strcmp (istiled, "tiled")));
  ...
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (setup_stretched),
				(!strcmp (istiled, "stretched")));
  ...
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (setup_auto),
				(strcmp (istiled, "tiled") && strcmp (istiled, "stretched")));
  ...
  gtk_drag_dest_set (preview_pixmap_frame,
		     GTK_DEST_DEFAULT_ALL,
		     xfbd_target_table, n_xfbd_targets,
		     GDK_ACTION_COPY | GDK_ACTION_MOVE);

  gtk_signal_connect (GTK_OBJECT (preview_pixmap_frame),
		      "drag_data_received",
...
void select_backdrp(char* theListName, char* theSelectedFile){
  FILE *theFile;
  char testStr[22];
  int Number, i;
  GList *files = NULL;
