...
GtkWidget *create_xfbdmgr(){
  ...
  gtk_drag_dest_set (clist_fileslist,
		     GTK_DEST_DEFAULT_ALL,
		     xfbdmgr_target_table, n_xfbdmgr_targets,
		     GDK_ACTION_COPY | GDK_ACTION_MOVE);

  gtk_signal_connect (GTK_OBJECT (clist_fileslist),
		      "drag_data_received",
		      GTK_SIGNAL_FUNC (on_drag_data_received),
		      (gpointer) NULL);
  gtk_signal_connect(GTK_OBJECT(edit_name), "changed", 
                     GTK_SIGNAL_FUNC(edit_changed), 
                     (gpointer) NULL);
  gtk_signal_connect(GTK_OBJECT(xfbdmgr), "destroy", 
                     GTK_SIGNAL_FUNC(Myquit), 
                     (gpointer) NULL);
  gtk_signal_connect(GTK_OBJECT(save_button), "clicked", 
                     GTK_SIGNAL_FUNC(save_clicked), 
                     (gpointer) NULL);
  gtk_signal_connect(GTK_OBJECT(load_button), "clicked", 
                     GTK_SIGNAL_FUNC(load_clicked), 
                     (gpointer) NULL);
  ...
  gtk_signal_connect(GTK_OBJECT(exit_button), "clicked", 
                     GTK_SIGNAL_FUNC(Myquit), 
                     (gpointer) NULL);
  gtk_signal_connect(GTK_OBJECT(clist_fileslist), "select_row", 
                     GTK_SIGNAL_FUNC(selectionMade), 
                     (gpointer) NULL);
  gtk_signal_connect(GTK_OBJECT(clist_fileslist), "unselect_row", 
                     GTK_SIGNAL_FUNC(selectionLost), 
                     (gpointer) NULL);
  return xfbdmgr;
}
